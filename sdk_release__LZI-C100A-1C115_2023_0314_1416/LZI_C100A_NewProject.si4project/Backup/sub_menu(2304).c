...
typedef struct tag_res_tab_t
{
	...
	sm_res_id_t  res_media_id[5][MAX_SUB_ITEM];//video photo music ebook
	...
	switch(smenu_attr->sub_menu_id)
	{
		...
		case ID_HOME_MUSIC:
		{
			if(smenu_attr->root_type == 0)
			{
				...
				smenu_attr->item_res_id[0] = sub_ui_para->res_media_id[smenu_attr->sub_menu_id - 0][0];
			}
			else
			{
				    ...
				    && (smenu_attr->root_type & 0xff00)) //TF
				{
					...
					smenu_attr->item_res_id[0] = sub_ui_para->res_media_id[smenu_attr->sub_menu_id - 0][1];
					smenu_attr->item_res_id[1] = sub_ui_para->res_media_id[smenu_attr->sub_menu_id - 0][2];
				}
				...
				{
					smenu_attr->item_nr = 1;
					smenu_attr->item_res_id[0] = sub_ui_para->res_media_id[smenu_attr->sub_menu_id - 0][2];
				}
				else
				{
					__err("root type err....\n");
...
static __s32 _sub_menu_Proc(__gui_msg_t *msg)
{
	...
	__wrn("_sub_menu_Proc msg->id=%d\n", msg->id);
	switch(msg->id)
	{
		case GUI_MSG_CREATE:	//子菜单创建
		{
			sub_menu_create(msg);
		}
		...
		case MSG_SUB_OP_ENTER:	//子菜单回车确认按键
		{
			smenu_attr_t *smenu_attr;
			smenu_attr = (smenu_attr_t *)GUI_WinGetAddData(msg->h_deswin);
			__wrn("msg->dwAddData2=%d\n", msg->dwAddData2);
			main_cmd2parent(msg->h_deswin, SMENU_APP_SWITCH, 0/*smenu_attr->focus_item*/, msg->dwAddData2);//smenu_attr->focus_item为0默认进u盘，为1默认进卡
		}
...
/*
	回调
*/
static __s32 _sub_menunumber_Proc(__gui_msg_t *msg)
{
	// __msg("_sub_menu_Proc msg->id=%d, msg->h_deswin=%x, msg->dwAddData1=%d, msg->dwAddData2=%d\n"
	//      ,msg->id, msg->h_deswin, msg->dwAddData1, msg->dwAddData2);
	__wrn("_sub_menunumber_Proc msg->id=%d\n", msg->id);
	switch(msg->id)
	{
		case GUI_MSG_CREATE:
		{
...
H_WIN sub_menu_winnumber_create(H_WIN h_parent, smenu_para_t *para)
{
	__gui_framewincreate_para_t framewin_para1;
	smenu_para_t *smenu_para;
	FB fb1;
	GUI_LyrWinGetFB(para->layer, &fb1);
	smenu_para = (smenu_para_t *)esMEMS_Balloc(sizeof(smenu_para_t));
	...
	smenu_para->root_type = para->root_type;
	smenu_para->focus_item = para->focus_item;
	eLIBs_memset(&framewin_para1, 0, sizeof(__gui_framewincreate_para_t));
	framewin_para1.name =	"sub_menunumbe_win",
	              framewin_para1.dwExStyle = WS_EX_NONE;
	framewin_para1.dwStyle = WS_NONE | WS_VISIBLE;
	framewin_para1.spCaption =  NULL;
	framewin_para1.hOwner	= NULL;
	framewin_para1.hHosting = h_parent;
	framewin_para1.id         = SUB_MENUNUMBER_ID;
	framewin_para1.FrameWinProc = (__pGUI_WIN_CB)esKRNL_GetCallBack((__pCBK_t)_sub_menunumber_Proc);
	framewin_para1.rect.x = 0;// 0;
	framewin_para1.rect.y = 0;//0;
	framewin_para1.rect.width =  fb1.size.width; //movie_layout.spsc_scnwin.width; //800
	framewin_para1.rect.height =  fb1.size.height;//movie_layout.spsc_scnwin.height;//300
	__wrn("framewin_para1 rect.x=%d, rect.y=%d, rect.width=%d, rect.height=%d\n",framewin_para1.rect.x,framewin_para1.rect.y,framewin_para1.rect.width,framewin_para1.rect.height);
	framewin_para1.BkColor.alpha =  0;
	framewin_para1.BkColor.red = 0;
	framewin_para1.BkColor.green = 1;
	framewin_para1.BkColor.blue = 0;
	framewin_para1.attr = (void *)smenu_para;
	framewin_para1.hLayer = para->layer;
	return (GUI_FrmWinCreate(&framewin_para1));
}
void  sub_menunumber_win_delete(H_WIN smenu_win)
{
	GUI_FrmWinDelete(smenu_win);
}
#endif
