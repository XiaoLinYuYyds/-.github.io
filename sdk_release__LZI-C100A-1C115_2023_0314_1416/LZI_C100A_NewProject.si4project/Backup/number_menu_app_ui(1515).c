...
*获取传入图片的数据 --> 传入图片ID索引
====================================================================================*/
static number_res_id_t *number_get_ui_para(__s32 rotate)
{
...
void paint_numbermain_one_app(numbermenu_attr_t *number_menu_attr, __s32 index, __s32 bfocus)
{
	...
	__u32 	i;
	void *app_pbmp;				//空指针
	__s32 app_bmp_width;		//app应用图片宽
	__s32 app_bmp_height;		//app应用图片高
	HBMP  app_hbmp;				//hbmp格式变量
	__s32 app_bmp_x;			//app应用图片x轴
	__s32 app_bmp_y;			//app应用图片y轴
	char  app_text[256];		//存放app应用显示字体文本数据
	home_uipara_t 		*home_ui_para;
	...
	#if 1	//绘制矩形
	{
		...
		__u16 xOft, yOft;
		xOft = (W_ITEM_FOCUS -ITEM_W)/2;						//选中的app应用图片宽190 - 没有选中的app应用图片宽 /2 = 矩形的x轴坐标
		yOft = (H_ITEM_FOCUS -ITEM_H)/2;						//选中的app应用图片高190 - 没有选中的app应用图片高 /2 = 矩形的y轴坐标
		...
		app_gui_rect.y0 = app_bmp_y +ITEM_H +24;				//app应用显示矩形y轴坐标 + 150(ITEM_H是没有选中的图片高)+ 24(与第二行的间隔高) = app应用矩形的起坐标y0
		app_gui_rect.x1 = app_gui_rect.x0 +ITEM_W -1 +32;		//app应用矩形的起坐标x0 + 150(ITEM_W是没有选中的图片宽) - 1 + 32 = app应用矩形的末尾坐标x1
		...
		else		//未选中图标
		{
			...
			GUI_ClearRect(app_bmp_x - yOft, app_bmp_y - yOft, app_bmp_x + W_ITEM_FOCUS - 1, app_bmp_y + H_ITEM_FOCUS - 1);	//删除选中过的app应用矩形显示
		#endif
		}
