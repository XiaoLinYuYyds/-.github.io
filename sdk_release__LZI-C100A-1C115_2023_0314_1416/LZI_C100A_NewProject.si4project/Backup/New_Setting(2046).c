...
*绘制单个应用
****************************************************************/
static __s32 New_Setting_paint_one_item(__gui_msg_t *msg,__u32 num)
{
	New_Setting_ctrl_t *New_Setting_ctrl;
	GUI_RECT gui_rect;
	...
	New_Setting_ctrl = (New_Setting_ctrl_t *)GUI_WinGetAddData(msg->h_deswin);
	GUI_LyrWinSel(New_Setting_ctrl->New_Settings_para->layer);
		...
		gui_rect.x0 = New_Setting_ctrl->New_Settings_uipara->numbers_tiltle_pos.x;		//矩形x 0
		gui_rect.y0 = New_Setting_ctrl->New_Settings_uipara->numbers_tiltle_pos.y;		//矩形y 0
		gui_rect.x1 = New_Setting_ctrl->New_Settings_uipara->numbers_tiltle_pos.width;	//矩形宽80
		gui_rect.y1 = New_Setting_ctrl->New_Settings_uipara->numbers_tiltle_pos.height;  //矩形高20
		{
...
static __s32 _New_Setting_proc(__gui_msg_t *msg)
{
	switch(msg->id)
	{
		case GUI_MSG_CREATE:	//创建窗口
		{
			...
			__wrn("GUI_MSG_CREATE 3 is ok\n");
			New_Setting_paint_one_item(msg,0);												//绘制单个图层
			__wrn("GUI_MSG_CREATE 4 is ok\n");
