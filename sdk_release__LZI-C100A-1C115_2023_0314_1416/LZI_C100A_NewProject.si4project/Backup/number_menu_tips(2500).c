...
//	H_THEME number_bmp;		//选中的主题，暂时不需要
//	H_THEME number_f_bmp;	//未选中的主题，暂时不需要
...
//绘制单个app应用图层
 __s32 app_menu_paint_one(__gui_msg_t *msg,__u32 num,__s32 index,__s32 focus_flage)
{
	number_menu_numbers_ctrl_t *numbers_ctrl;
	numbermenu_attr_t *numbermenu_attr;
	...
	//for(i = 0; i < APP_MAX_NUM;i++)
	{
		...
		GUI_BMP_Draw(app_theme,app_bmp_x,app_bmp_y);							//app应用图片绘制到矩形控件上面
		{
			...
			dsk_langres_get_menu_text(app_ui_id[index].app_lang_id, app_txt, sizeof(app_txt));//不结构体文本文字ID索引赋值给app_txt数组
 ...
 __s32 number_menu_numbers_paint_all(__gui_msg_t *msg,__u32 num)
{
	...
	GUI_LyrWinSel(numbers_ctrl->numbers_para->layer);			//先选择图层窗口事件，选择窗口图层然后在图层上面绘制，numbers_ctrl->numbers_para->layer
		...
		gui_rect.x0 = 0+numbers_ctrl->numbers_uipara->numbers_tiltle_pos.x;		//矩形x
		gui_rect.y0 = 0+numbers_ctrl->numbers_uipara->numbers_tiltle_pos.y;		//矩形y
		gui_rect.x1 = numbers_ctrl->numbers_uipara->numbers_tiltle_pos.width;	//矩形宽150
		gui_rect.y1 = numbers_ctrl->numbers_uipara->numbers_tiltle_pos.height;  //矩形高150
		{
			__wrn("gui_rect.x0 = %d , gui_rect.y0 = %d,gui_rect.width = %d , gui_rect.heigh = %d\n",gui_rect.x0,gui_rect.y0,gui_rect.x1,gui_rect.y1);
...
static __s32 number_menu_numbers_proc(__gui_msg_t *msg)
{
	...
	switch(msg->id)
	{
		...
		case GUI_MSG_DESTROY:	//销毁窗口
		{
			...
			#else
			theme_close(numbers_ctrl->app_focus_bmp[APP_MAX_NUM]);		//选中额app应用图片关闭
			theme_close(numbers_ctrl->app_unfocus_bmp[APP_MAX_NUM]);		//未选中的app应用图片关闭
			#endif
		...
		case MSG_NUMBER_MENU_OP_DOWN:		//下
		{
			...
			#if 1
				app_menu_paint_one(msg, 0,key_down_number,0);
				key_up_number++;

				if(key_down_number > APP_MAX_NUM)
				{
					key_down_number = 0;
				}

				app_menu_paint_one(msg, 0,key_up_number, 1);
			#endif
		}
		return EPDK_OK;
		case MSG_NUMBER_MENU_OP_UP:		//上
		{
			...
			#if 1
				app_menu_paint_one(msg, 0,key_up_number,0);
				key_up_number--;
