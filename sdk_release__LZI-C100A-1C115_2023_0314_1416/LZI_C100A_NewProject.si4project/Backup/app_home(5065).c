				...
				case SUB_MENU_ID:	//子菜单ID，意思是在进入子菜单后进行执行下面的程序
				{
					if(HIWORD(msg->dwAddData1) == SMENU_APP_SWITCH)
					{
						...
						switch(home_para->focus_id)
						{
							...
							case ID_HOME_NEW_SETTING_NUMBER://新添加APP应用窗口
							{
								...
								//GUI_LyrWinSetSta(home_para->lyr_total_bg, GUI_LYRWIN_STA_SUSPEND);
								main_menu_uninit_res(home_para->h_mmenu);
								main_cmd2parent(msg->h_deswin, SWITCH_TO_OTHER_APP, ID_HOME_NEW_SETTING_NUMBER, 0);//进入通用设置子菜单
								break;
							}
				...
				break;
					...
					case SUB_MENUNUMBER_ID://进入新主菜单空白窗口ID
					{
						...
						{	
							...
							case ID_HOME_NEW_SETTING_NUMBER://进入//直接发送新添加的通用设置app的id创建mainwin窗口
							{
								#if 1
