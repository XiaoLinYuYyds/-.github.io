...
{
	SIZE screen_size;
	dsk_display_get_size(&screen_size.width, &screen_size.height);		//获取显示尺寸字节大小
	numbers_ctrl->numbers_uipara = number_get_numbers_para(screen_size);//获取对话框框大小
...
static __s32  number_menu_numbers_key_proc(__gui_msg_t *msg)
{
	...
	{
		__wrn("number_menu_numbers msg->dwAddData2 = %d\n",msg->dwAddData2);
	...
	else
	{
		...
		{
		__wrn("number_menu_numbers msg->dwAddData1 = %d\n",msg->dwAddData1);
			case GUI_MSG_KEY_LEFT:		//为up
			case GUI_MSG_KEY_LONGLEFT:
			{
				main_cmd2parent(msg->h_deswin, ID_OP_UP, 0, 0);//命令控制
			}
			break;
...
static __s32 number_menu_numbers_proc(__gui_msg_t *msg)
{
	...
	switch(msg->id)
	{
		...
		case GUI_MSG_CREATE:	//创建菜单窗口
		{
			number_menu_numbers_ctrl_t *numbers_ctrl;
			number_menu_number_para_t *numbers_para;
			numbers_ctrl = (number_menu_numbers_ctrl_t *)My_Balloc(sizeof(number_menu_numbers_ctrl_t));
			...
			eLIBs_memset(numbers_ctrl, 0, sizeof(number_menu_numbers_ctrl_t));	//内存分配
			numbers_para = (number_menu_number_para_t *)GUI_WinGetAttr(msg->h_deswin);
			...
			number_menu_number_init(numbers_ctrl);					//
		//	GUI_LyrWinCacheOn();	//窗口检查打开
			GUI_LyrWinSetSta(numbers_ctrl->numbers_para->layer, GUI_LYRWIN_STA_ON);	//窗口设置开启状态
			GUI_LyrWinSetTop(numbers_ctrl->numbers_para->layer);
			number_menu_numbers_paint_all(msg,0);//窗口设置停止状态
...
H_WIN number_menu_numbers_win_create(H_WIN h_parent, number_menu_number_para_t *para)
{
	...
	numbers_para = (number_menu_number_para_t *)esMEMS_Balloc(sizeof(number_menu_number_para_t));
	...
	numbers_para->sub_menu_id = para->sub_menu_id;	//子菜单ID
	...
	framewin_para.id         = SUB_MENUNUMBER_ID;	//GENERAL_TIPS_ID;//SUB_MENU_ID;//GENERAL_TIPS_ID;//	//窗口ID,作用返回上一层需要用到
