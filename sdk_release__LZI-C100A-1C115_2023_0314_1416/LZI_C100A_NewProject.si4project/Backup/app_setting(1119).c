				...
				case GENERAL_MENU_ID:
				{
					switch(HIWORD(msg->dwAddData1))
					{
						...
						case ID_SWITCH_TO_TIPS:
						{
							...
							#else
							__wrn("number_menu_numbers key_number = %d\n",key_number);
							// 回车按键点击次数窗口提示框
							
							key_number += 1;
							if(key_number >= 3)//点击三次才进入
							{
								key_number = 0;
								rect.x = 0;
								rect.y = 0;
								...
								rect.height = 101;
								tips_para.msgbox_font = setting_ctrl->setting_font;
								tips_para.layer = number_menu_8bpp_layer_create(&rect,1);
								setting_ctrl->h_frm_tips = number_menu_numbers_win_create(GUI_WinGetHandFromName(APP_SETTING), &tips_para);
								GUI_WinSetFocusChild(setting_ctrl->h_frm_tips);
							}
							#endif
							}
...

{
	...
	create_info.name            = APP_NEWSETTING;//APP_SETTING;
	...
	create_info.id			= APP_NEWSETTING_ID;//APP_SETTING_ID;
