...
//当前找到一个有效频率
static void on_new_fm_test_freq_ok(FM_CTRLDATA_T *fm_ctrl)
{
...
************************************************************************************************************************
*/
__s32 cd_srhch_success(void *arg_p)
{
	NEW_FM_WND_T *wnd_para;
	H_WIN hwnd = (H_WIN)arg_p;
	FM_GetWndPara( wnd_para, NEW_FM_WND_T, hwnd);

	if(wnd_para)
	{
		on_new_fm_test_freq_ok(FM_CTRLDATA_T * fm_ctrl);
	}
}
