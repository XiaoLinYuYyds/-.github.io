...
static __s32 app_new_ebook_framewin_create(__gui_msg_t *msg)
{
	...
	__s32               tmp_point;
	char                path[1024];
	app_new_ebook_ctrl_t   *ebook_ctrl;
	RECT 	rect;
	__s32 	ret;
	__u8    err;
	ebook_ctrl = (app_new_ebook_ctrl_t *)GUI_WinGetAttr(msg->h_deswin);
	...
	/* 创建 mbook core 句柄 */
	err = 0;
	ebook_ctrl->mbook = MBOOK_Decode_Init(NULL, &err);//电子书解码初始化
	...
	// 设置默认编码
	MBOOK_Decode_SetDefaultCharset(ebook_ctrl->mbook, EPDK_CHARSET_ENM_GBK);//默认为GBK编码格式
	{
		ebook_ctrl->config.font_color    = APP_COLOR_WHITE;//文本颜色白色
	}
	/************************************文本显示控制****************************/
	ebook_ctrl->config.show_rotate   = 0;
	ebook_ctrl->config.font_size     = 16;
	ebook_ctrl->config.char_font     = ebook_ctrl->font;
	ebook_ctrl->config.row_space     = 10;
	ebook_ctrl->config.col_space     = 2;
	...
	switch(msg->id){
		case GUI_MSG_CREATE:{/*0*/
			...
			com_set_palette_by_id(ID_EBOOK_WKM_BMP);		//调色板id
			{
				...
				new_ebook_GetListItemFileName(msg, 22, FileName);	//获取列表项目的图片文件名
				__wrn("\nFileName = %s\n",FileName);
				PlayEbookFiles(msg, 22, FileName);//播放文件
			#endif
