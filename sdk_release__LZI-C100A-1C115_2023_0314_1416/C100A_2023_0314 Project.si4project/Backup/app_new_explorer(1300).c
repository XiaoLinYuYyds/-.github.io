...
static __s32 new_explorer_scene_create(__gui_msg_t *msg)
{
	...
	__wrn("new_explorer_ListPara. size = %d\n",sizeof(new_explorer_list_para_t));
...
static __s32 _new_explorer_manager_win_proc(__gui_msg_t *msg)
{
	...
	switch(msg->id)
	{
		...
		case GUI_MSG_QUIT:
		{
			...
			__wrn("msg->dwAddData2 is  = %d....\n",msg->dwAddData2);
			if(0 == msg->dwAddData2)//0伪退出到home界面
				app_new_explorer_cmd2parent(msg->h_deswin, NEW_SWITCH_TO_OTHER_APP, NEW_SETTING_SW_TO_MAIN, 0);//发送到app_root_scene.c 的case APP_NEWSETTING_ID:条件下执行跳转切换到app_setting.c 的创建app应用manwin窗口界面
			else if(1 == msg->dwAddData2)//1伪退出到new_movie应用程序
		...
		case GUI_MSG_KEY:		//manwin窗口按键功能
		{
				...
				__gui_msg_t my_msg;
				__wrn("new explorer GUI_MSG_KEY  send GUI_MSG_QUIT is...\n");
		...
		case GUI_MSG_COMMAND:	//接收到framewin窗口发送的命令
		{
			...
			switch(LOWORD(msg->dwAddData1))
			{
				...
				case NEW_NEWEXPLORER_LIST_ID://0x9700
				{
					...
					{
						case ID_OP_NEW_EXPLORER_EXIT://发送关闭命令，并退出到home界面
						{
							...
							my_msg.h_deswin = msg->h_deswin;	//app_new_explorer.c父窗口manwin窗口发送退出信号
							my_msg.dwAddData1 = 0;
							my_msg.dwAddData2 = 0;//app切换到home解决
							GUI_SendNotifyMessage(&my_msg);		//发送通知信息到当前manwin窗口
						...
						case ID_OP_NEW_EXPLORER_ENTER://确认按键按下发送关闭命令，并切换到new_movie应用程序
						{
							
							#if 1//发送按键信号到当前的APP_NEWSETTING_ID父窗口manwin窗口的写法
							__gui_msg_t my_msg;
							__wrn("new explorer  ID_OP_NEW_EXPLORER_ENTER is...\n");
							__wrn("msg->dwAddData2 = %d is......\n",msg->dwAddData2);
							my_msg.id = GUI_MSG_QUIT;			//当前文件manwin窗口的退出窗口id
							my_msg.h_srcwin = msg->h_srcwin;	//app_new_explorer.c父窗口manwin窗口发送退出信号
							my_msg.h_deswin = msg->h_deswin;	//app_new_explorer.c父窗口manwin窗口发送退出信号
							my_msg.dwAddData1 = msg->dwAddData2;//播放文件索引id
							if(new_explorer_ctrl->media_type == RAT_MEDIA_TYPE_VIDEO){//搜索的媒体类型为视频文件
								my_msg.dwAddData2 = 1;//app界面切换到new_movie应用程序
							}
						...
						case ID_OP_NEW_EXPLORER_RIGHT://右移,为上按键功能
						{
							__gui_msg_t mymsg;
							mymsg.h_deswin = new_explorer_ctrl->h_new_explore;//向new_explorer_list.c的framewin窗口发送按键信号
							mymsg.h_srcwin = NULL;
							mymsg.id = MSG_NEW_EXPLORER_OP_RIGHT;
							__wrn("MSG_NEW_EXPLORER_OP_RIGHT NEW \n");
							GUI_SendMessage(&mymsg);//向new_list_layer图层发送消息命令
						}
						break;

						case ID_OP_NEW_EXPLORER_LEFT://左移，为下按键功能
						{
							__gui_msg_t mymsg;
							mymsg.h_deswin = new_explorer_ctrl->h_new_explore;//向new_explorer_list.c的framewin窗口发送按键信号
							mymsg.h_srcwin = NULL;
							mymsg.id = MSG_NEW_EXPLORER_OP_LEFT;
							__wrn("MSG_NEW_EXPLORER_OP_RIGHT NEW \n");
							GUI_SendMessage(&mymsg);//向new_list_layer图层发送消息命令
						}
						break;
...
H_WIN app_new_explorer_manwin_create(root_para_t *para)
{
	...
	new_explorer_ctrl = (new_explorer_ctrl_t *)My_Balloc(sizeof(new_explorer_ctrl_t));//为私有属性结构体申请内存空间
	if(!new_explorer_ctrl)
	{
	...
	switch(para->data)
	{
		...
		case ID_NEW_EXPLORER_MUSIC://3时，执行搜索磁盘的音频文件功能
		{
			new_explorer_ctrl->view_mode  = NEW_EXPLR_LIST_MODE;//EXPLR_LIST_MODE;
			new_explorer_ctrl->media_type = RAT_MEDIA_TYPE_AUDIO;//获取磁盘的音频文件
			new_explorer_ctrl->root_type  = para->root_type;	 //检测是USB还是SD卡磁盘
		}
		break;

		case ID_NEW_EXPLORER_PHOTO://2时，执行搜索磁盘的图片文件功能
		{
			new_explorer_ctrl->view_mode  = NEW_EXPLR_LIST_MODE;//EXPLR_LIST_MODE;
			new_explorer_ctrl->media_type = RAT_MEDIA_TYPE_PIC; //获取磁盘的所有图片文件
			new_explorer_ctrl->root_type  = para->root_type;	//检测是USB还是SD卡磁盘
		}
		break;
