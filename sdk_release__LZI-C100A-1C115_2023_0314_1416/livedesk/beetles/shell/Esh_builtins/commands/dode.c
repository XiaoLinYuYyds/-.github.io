	...
	if(Esh_strcmp(args[1], "info") == 0 || Esh_strcmp(args[1], "INFO") == 0)
	{
		...
		for(i = 0; i < 4; i++)
		{
			...
			else if(pixel_fmt == DISP_FORMAT_YUV422 && pixel_mode == DISP_MOD_INTERLEAVED)
			{
				...
				{
					Esh_printf("\tpixels sequence:UYVY,");
				}
				else if(pixel_seq == DISP_SEQ_YUYV)
				{
					Esh_printf("\tpixels sequence:YUYV,");
				}
				else if(pixel_seq == DISP_SEQ_VYUY)
				{
					Esh_printf("\tpixels sequence:VYUY,");
				}
				else if(pixel_seq == DISP_SEQ_YVYU)
				{
					Esh_printf("\tpixels sequence:YVYU,");
				}
			}
			else if(pixel_fmt == DISP_FORMAT_YUV444 && pixel_mode == DISP_MOD_INTERLEAVED)
			{
				...
				{
					Esh_printf("\tpixels sequence:AYUV,");
				}
				else if(pixel_seq == DISP_SEQ_VUYA)
				{
					Esh_printf("\tpixels sequence:VUYA,");
				}
			}
			else if(pixel_fmt == DISP_FORMAT_YUV420 && (pixel_mode == DISP_MOD_NON_MB_UV_COMBINED || pixel_mode == DISP_MOD_MB_UV_COMBINED))
			{
				...
				{
					Esh_printf("\tpixels sequence:UVUV,");
				}
				else if(pixel_seq == DISP_SEQ_VUVU)
				{
					Esh_printf("\tpixels sequence:VUVU,");
				}
			}
			else
			{
