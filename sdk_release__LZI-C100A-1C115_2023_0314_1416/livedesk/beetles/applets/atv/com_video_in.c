...
static __s32	g_to_quit_video = 1;

static ES_FILE *g_fs_mp = NULL;
mp_rotate_mirror g_mp_rotate;
...
void DeSetSmooth(__disp_video_smooth_t mode)
{
	__u32 arg[3]={0};
    ...
    arg[1] = mode;
    arg[2] = 0;
    eLIBs_fioctrl(video_ctrl.disphd, DISP_CMD_LAYER_SET_SMOOTH, 0, (void*)arg);

	return;
}


__s32 com_video_Color_Setting(void)
{	
	com_video_set_contrast(90);
	com_video_set_bright(57);
	com_video_set_saturation(110);
	com_video_set_hue(41);
	com_video_set_chrom_gain(65);
	DeSetSmooth(DISP_VIDEO_VERYSHARP);

	return EPDK_OK;
}

extern __s8			g_isAVInput;
__s32 video_dev_init(video_source_e video_source, __s32 tvd_sel,  __drv_TVD_system cvbsSys, __s32 enable_3d)
{
    __s32 i;
    __u32 mod_num=0;
    ...
    //先初始化
    eLIBs_fioctrl(video_ctrl.pvideo_dev, DRV_TVD_CMD_INIT, tvd_sel, 0);

#if 0
	config_para.system = 0;
	esKRNL_TimeDly(10);
	system = eLIBs_fioctrl(video_ctrl.pvideo_dev, DRV_TVD_CMD_GET_ADC_STATUS, tvd_sel, 0);
	__msg_jjl("system=%x\n",system);
	if (system != EPDK_FAIL)
	{
		if (system & 0x1) 
...
#else
	config_para.system = cvbsSys;
#endif

	config_para.tvd_channel = tvd_sel;
  ...
  if(!g_isAVInput)
  {
    com_video_set_gain(0x0B>>1);
  }
  else
  {
    com_video_set_gain(0x10>>1);
  }
...
{
	if(!g_to_quit_video)	return EPDK_OK +1;
	
	eLIBs_memset((void*)&video_ctrl, 0 , sizeof(__com_video_ctrl_t));

	DebugPrintf("com_video_in_open:dev_index=%d,tvd_sel=%d,cvbsSys=%d,enable_3d=%d\n",dev_index,tvd_sel,cvbsSys,enable_3d);
	...
	video_dev_init(video_ctrl.video_source, tvd_sel, cvbsSys, enable_3d);

	//com_video_Color_Setting();
...
{
    __wrn("com_video_in_set_sel, tvd_sel:%d\n", tvd_sel);
    
    if(video_ctrl.pvideo_dev)
    {
        eLIBs_fioctrl(video_ctrl.pvideo_dev, DRV_TVD_CMD_SET_TV_SEL, tvd_sel, 0);
    }
...
__s32 com_video_set_gain(__s32 val) // TV:5; 其它:8.
{// 16:TVIN gain debug; 17~21:TVIN gain val.
    __u32 v = REG_RD32(TVD_REGS_BASE+0x0e2c);
    
    v &= ~(0x1f<<17);

    val &= 0x1f;
    v |= (val<<17);
    REG_WR32(TVD_REGS_BASE+0x0e2c, v);
...
{
	__u16 i, cnt = 0, format, format_bk = 0xFF;
	for(i = 0; i < (400); i++)
	{
		if(com_video_get_status())
		{
			format = com_video_get_format();
			__msg_jjl("fmt=%d\n", format);
			if(format == format_bk)
			{
				if(++cnt >= 5)	return format;
			}
			else
			{
				format_bk = format;
