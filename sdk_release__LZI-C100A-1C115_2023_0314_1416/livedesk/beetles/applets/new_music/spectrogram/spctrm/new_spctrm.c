...
static void _new_update_spectrum(void)
{
	...
	__s32  ret;
	ret = robin_get_spectrum_info(temp_level_array);//获取频谱即时信息
	__msg("_new_update_spectrum ret=%d...\n",ret);
	//ret = dsk_wkm_get_spectrum_info(temp_level_array);
	if(ret)//获取频谱即时信息失败
	{
		__wrn("No spectrum.\n");
		NEW_SPECTRUM_SetValue(hwin_spectrum, NULL);//设置频谱设定值
	}
	else
	{//获取频谱即时信息成功
		__u32  i;
		__msg("_new_update_spectrum real_level_array...\n");
