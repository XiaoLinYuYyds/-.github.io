...
#define		send_cmd_play_prev_to_PlayDev	0//从播放器发送播放上一首音乐命令到播放列表
#define		send_cmd_play_next_to_PlayDev	1//从播放器发送播放下一首音乐命令到播放列表
#define		send_cmd_playlist_exit			3//从播放列表发送退出本窗口命令

#define		playlist_send_cmd_prev_play		4//从播放列表窗口按键操作发送播放上一首命令到播放器
#define		playlist_send_cmd_next_play		5//从播放列表窗口按键操作发送播放上一首命令到播放器
...
{
	H_WIN	playlist_hfrm;	//存放播放列表framewin窗口内部的句柄
	H_LYR	playlist_hlyr;	//存放播放列表framewin窗口内部的图层
	GUI_FONT *font;			//存放播放列表文本
...
H_WIN	playlist_frmwin_create(H_WIN h_parent, playlist_create_para_t *para);//播放列表framewin窗口创建
