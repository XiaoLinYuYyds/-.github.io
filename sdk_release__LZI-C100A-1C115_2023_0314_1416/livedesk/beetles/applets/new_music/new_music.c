		...
		}
		{
			...
			{//绘制进度条移动
				...
				if(cur_time <= 0 || total_time <= 0){//当前播放时间或总时间小于等于0时
					...
					__here__;
					n = (((new_music_ctrl->new_music_uipara->progressbar[0].width+62+35) * cur_time)/*进度条背景图片的宽(276)*当前时间*/
				...
				for(i = 0 ; i < n ; i++){
					...
					__msg(" ui_para->uipara_progressbar_icon.x + i * ui_para->uipara_progressbar_icon.w= %d \n",  
...
static __s32  new_music_play_ctrl_key_proc(__gui_msg_t *msg)
{
	...
	if(KEY_UP_ACTION == msg->dwAddData2){//按键松开动作获取，第一次进入的按键执行功能
			__wrn("last_key=%d\n", last_key);
			switch(last_key){
				case GUI_MSG_KEY_ENTER:{//回车播放按键按下，松开后的操作
				...
				case GUI_MSG_KEY_LONGRIGHT:{//103，长按下一首，松开后检测到快进状态发送播放，继续播放
				...
				case GUI_MSG_KEY_LONGLEFT:{//104，长按上一首，松开后检测到后退状态发送播放，继续播放
					__cedar_status_t sta;
				...
				case GUI_MSG_KEY_MENU:{//按下返回按键，松开后发送退出命令到父窗口manwin窗口
		...
		switch(msg->dwAddData1){
			...
			case GUI_MSG_KEY_LONGRIGHT:{	//长按下一首，快进
				...
				if(CEDAR_STAT_PLAY == sta)
				{
					...
					this->framewin_create_para->lastplay_index -= 1;//文件索引自减1，作用是消除长按时触发的短按文件索引减1.使播放的文件和播放列表更新的选中文件一致
					if(this->framewin_create_para->lastplay_index < 0){
							__wrn(" this->framewin_create_para->lastplay_index =%d...\n",this->framewin_create_para->lastplay_index);
							this->framewin_create_para->lastplay_index = 16;
							last_index = 0;
							last_index = this->framewin_create_para->lastplay_index;
					}
			...
			case GUI_MSG_KEY_LONGLEFT:{		//长按上一首，后退
				...
				if(CEDAR_STAT_PLAY == sta)
				{
					...
					this->framewin_create_para->lastplay_index += 1;//文件索引自增1，作用是消除长按时触发的短按文件索引加1.使播放的文件和播放列表更新的选中文件一致
					if(this->framewin_create_para->lastplay_index > 16){
							__wrn(" this->framewin_create_para->lastplay_index =%d...\n",this->framewin_create_para->lastplay_index);
							this->framewin_create_para->lastplay_index = 0;
							last_index = 0;
							last_index = this->framewin_create_para->lastplay_index;
					}
					last_index = this->framewin_create_para->lastplay_index;
...
static __s32 __new_music_proc(__gui_msg_t *msg)
{
	...
	switch(msg->id){
		...
		case GUI_MSG_TIMER:{/*19，定时器*/
			...
			if(new_music_ctrl)
			{
				play_index = new_music_ctrl->framewin_create_para->lastplay_index ;//把上一次的播放文件索引id赋值给play_index变量
				__msg("play_index 100 = %d...\n",play_index);					
							...
							new_music_progressbar_move_paint(msg);		//绘制进度条
								...
								__wrn("play_index = %d...\n",play_index);
									...
									if(key_longup_action_flag == 1){//长按上一首标志，后退时更新播放列表为上一首
										...
										new_music_play_start(new_music_ctrl,play_index);//直接开始播放音乐文件，不更新播放列表
										//app_new_music_cmd2para(msg->h_deswin, MSG_PLAY_OP_DOWN, play_index, 0);	//向父窗口manwin窗口发送命令上一首命令
									}
