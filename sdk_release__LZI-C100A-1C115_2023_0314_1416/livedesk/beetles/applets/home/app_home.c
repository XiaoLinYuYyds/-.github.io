...
{	
	...
	home_ui_para = home_get_ui_para(GUI_GetScnDir());//获取窗口大小 number_get_numbers_para(GUI_GetScnDir());//
	if(!home_ui_para)
	{
	...
	#else	//通过获取lcd显示屏尺寸的大小
	rect.x = home_ui_para->lyr_sub_item.x;		//设置主窗口x轴坐标 0
	rect.y = home_ui_para->lyr_sub_item.y;		//设置主窗口y轴坐标 0
	rect.width = home_ui_para->lyr_sub_item.w;	//设置主窗口宽	原始的1024
	rect.height = home_ui_para->lyr_sub_item.h;//设置主窗口高 原始的600
	...
	home_para->lyr_smenu = home_8bpp_layer_create(&rect,1);//图层创建
	if(!home_para->lyr_smenu)//检测图层是否创建成功
	{
		__err("home_para->lyr_smenu is null\n");
		return;
	}
	new_smenu_para.layer = home_para->lyr_smenu;				//一图层赋值，没有选中app应用前的图层
	#if 0
	new_smenu_para.sub_menu_id = home_para->focus_id;			//选中的app应用窗口ID
	new_smenu_para.smenu_font = home_para->main_font;			//字体
	new_smenu_para.root_type = home_para->root_type;			//
	new_smenu_para.focus_item = home_para->focus_smenu_item;	//选中的内容
	new_smenu_para.bg_layer = home_para->lyr_smenu;				//二图层，选中app应用的的背影图层
	#endif
	...
	__msg("home_para->h_submenu=%x\n", home_para->h_submenu);
     GUI_WinSetFocusChild(home_para->h_submenu);			//命令发送，按键执行需要调用
...
{	
	...
	rect.height = 480;//home_ui_para->lyr_sub_item.h;
	#else	//通过获取lcd显示屏尺寸的大小
	rect.x = home_ui_para->lyr_main_menu.x;		//设置主窗口x轴坐标
	rect.y = home_ui_para->lyr_main_menu.y;		//设置主窗口y轴坐标
	rect.width = home_ui_para->lyr_main_menu.w;	//设置主窗口宽	原始的1024
	rect.height = home_ui_para->lyr_main_menu.h;//设置主窗口高 原始的600
	__wrn("rect.x=%d, rect.y=%d, rect.width=%d, rect.height=%d\n", rect.x, rect.y, rect.width, rect.height);
	#endif
	...
	__wrn("newsub_menu rect.x=%d, rect.y=%d, rect.width=%d, rect.height=%d\n",
	      rect.x, rect.y, rect.width, rect.height);
	home_para->lyr_mmenu = number_menu_32bpp_layer_create(&rect,1);//图层创建
	if(!home_para->lyr_mmenu)//检测图层是否创建成功
	{
		__err("home_para->lyr_smenu is null\n");
		return;
	}
	smenu_para.layer = home_para->lyr_mmenu;				//一图层赋值，没有选中app应用前的图层
	smenu_para.sub_menu_id = home_para->focus_id;			//选中的app应用窗口ID
	smenu_para.smenu_font = home_para->main_font;			//字体
	smenu_para.root_type = home_para->root_type;			//
	smenu_para.focus_item = home_para->focus_smenu_item;	//选中的内容
	smenu_para.bg_layer = home_para->lyr_mmenu;				//二图层，选中app应用的的背影图层
	...
	__msg("home_para->h_mmenu=%x\n", home_para->h_mmenu);
     GUI_WinSetFocusChild(home_para->h_mmenu);			//命令发送，按键执行需要调用
	if(!home_para->h_mmenu)								//检测窗口是否创建成功
	{
...
static __s32 app_home_proc(__gui_msg_t *msg)
{
	...
	switch(msg->id)
	{
		case GUI_MSG_CREATE:	//菜单创建
		{
			...
			#else//新的主窗口
			app_newsub_menu_create(home_para);//新添加主窗口
			#endif
			//建立一个子菜单
			...
			//app_newsetting_sub_menu_create(home_para);//新添加通用设置app子窗口
			#endif
#if 1 == HOME_USE_HOME_LOGO
			GUI_LyrWinSetSta(home_para->lyr_mmenu, GUI_LYRWIN_STA_ON);	//主菜单图层窗口状态打开
#else
#endif
			#if 1//原来的
			GUI_LyrWinSetSta(home_para->lyr_smenu, GUI_LYRWIN_STA_ON);		//设置子菜单图层窗口状态
		...
		case GUI_MSG_COMMAND:	//命令输入
		{
			...
			switch(LOWORD(msg->dwAddData1))
			{
				...
				case SUB_MENU_ID:	//子菜单ID，意思是在进入子菜单后进行执行下面的程序
				{
					if(HIWORD(msg->dwAddData1) == SMENU_APP_SWITCH)
					{
						...
						switch(home_para->focus_id)
						{
							...
							case ID_HOME_CALENDAR://进入日历app应用
							{
								GUI_LyrWinSetSta(home_para->lyr_smenu, GUI_LYRWIN_STA_SUSPEND);
								GUI_LyrWinSetSta(home_para->lyr_mmenu, GUI_LYRWIN_STA_SUSPEND);
								GUI_LyrWinSetSta(home_para->lyr_forground, GUI_LYRWIN_STA_SUSPEND);
							...
							case ID_HOME_GENERAL_SETTING://进入通用设置app应用程序创建manwin窗口
							{
								GUI_LyrWinSetSta(home_para->lyr_smenu, GUI_LYRWIN_STA_SUSPEND);
								GUI_LyrWinSetSta(home_para->lyr_mmenu, GUI_LYRWIN_STA_SUSPEND);
								GUI_LyrWinSetSta(home_para->lyr_forground, GUI_LYRWIN_STA_SUSPEND);
							...
							case ID_HOME_NEW_SETTING_NUMBER://新添加跳转到app_root_scene.c去创建APP应用manwin窗口
							{
								__wrn("ID_HOME_NEW_SETTING_NUMBER is...\n");
								GUI_LyrWinSetSta(home_para->lyr_smenu, GUI_LYRWIN_STA_SUSPEND);	//图层窗口暂停状态
								GUI_LyrWinSetSta(home_para->lyr_mmenu, GUI_LYRWIN_STA_SUSPEND);
								GUI_LyrWinSetSta(home_para->lyr_forground, GUI_LYRWIN_STA_SUSPEND);
								//GUI_LyrWinSetSta(home_para->lyr_total_bg, GUI_LYRWIN_STA_SUSPEND);
								main_menu_uninit_res(home_para->h_mmenu);
								main_cmd2parent(msg->h_deswin, SWITCH_TO_OTHER_APP, ID_HOME_NEW_SETTING_NUMBER, 0);//进入新的app应用子菜单
								break;
							}
							case ID_HOME_NEW_EXPORER://新添加跳转到app_root_scene.c去创建new_explorer的APP应用manwin窗口
							{
								check_disk(home_para);//检查是否存在磁盘
								...
								if(home_para->root_type != 0)//等于2为TF卡,2<<8等于512:二进制为 0010 0000 0000
								{
									...
									main_cmd2parent(msg->h_deswin, SWITCH_TO_OTHER_APP, ID_HOME_NEW_EXPORER, 2);	//发送到app_root_scene.c文件进入新的new_explorer创建manwin窗口功能；2为TF卡
							...
							case ID_HOME_NEW_MOVIE:/*新添加跳转到app_root_scene.c去创建new_movie的APP应用manwin窗口*/
							{
								check_disk(home_para);//检查是否存在磁盘
							...
							case ID_HOME_NEW_PHOTO:{//新添加相册app应用
								check_disk(home_para);//检查是否存在磁盘
							...
							case ID_HOME_NEWMUSIC://新添加的music应用
							{
								check_disk(home_para);//检查是否存在磁盘
				...
				break;
				#if 1	//新添加主菜单和子菜单窗口
					#if 0
					case SUB_MENUNUMBER_ID://进入新主菜单空白窗口ID
					{
						#if 1
						...
						{	
							#if 1
							case ID_NUMBER_MENU_SWITCH_ITEM://子菜单创建0
							{
								//先挡住子菜单，解决子菜单切换时闪的问题。
								//GUI_LyrWinSetTop(home_para->lyr_mmenu);//112357
								//gscene_hbar_set_state(HBAR_ST_SHOW);//112357
								...
								home_para->focus_id = msg->dwAddData2;
								home_para->first_item = msg->dwReserved;
								//app_sub_menu_create(home_para);								 //在home主菜单界面窗口在创建子菜单
								app_newsetting_sub_menu_create(home_para);					//新添加的子菜单
								GUI_LyrWinSetSta(home_para->lyr_smenu, GUI_LYRWIN_STA_SLEEP);//图层窗口设置为休眠状态
								//GUI_LyrWinSetTop(home_para->lyr_smenu);
							}
							break;
							#endif
							...
							case ID_OP_NUMBER_MENU_ENTER:	//回车按键3
							{
								...
								GUI_SendMessage(&mymsg);
								#else	//直接发送新添加的通用设置app的id创建mainwin窗口
							...
							case ID_HOME_NEW_SETTING_NUMBER://进入直接发送新添加的通用设置app的id创建mainwin窗口
							{
								#if 1
					...
					#endif
					#if 0 //新的通用设置子菜单窗口ID
					case NEW_SETTING_ID://进入新子菜单空白窗口ID
					{	
						#if 1
						switch(HIWORD(msg->dwAddData1))
						{	
							...
							case ID_HOME_NEW_SETTING_NUMBER://新添加通用设置app的ID
							{
								if(home_para->lyr_smenu)
								{
									GUI_LyrWinSetSta(home_para->lyr_smenu, GUI_LYRWIN_STA_SUSPEND);
