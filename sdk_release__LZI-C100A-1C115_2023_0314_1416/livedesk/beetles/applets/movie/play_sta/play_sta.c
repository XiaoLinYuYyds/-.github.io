...
static __s32 __playsta_update_playsta_ui(movie_playsta_scene_t *scene_para)
{
	...
	char item_str[1][128];
	GUI_RECT gui_rect;
	__s32 lang_id_play = STRING_MOVIE_ST_PLAT;
	__s32 lang_id_stop = STRING_MOVIE_ST_STOP;
	ui_para = movie_playsta_get_uipara(GUI_GetScnDir());
...
#if 1 == SHOW_PLAYPAUSE_TEXT
	{
		...
		if(movie_playsta_pause == scene_para->playsta)
		{
			dsk_langres_get_menu_text(lang_id_stop, item_str, GUI_TITLE_MAX);
		}
		else
		{
			dsk_langres_get_menu_text(lang_id_play, item_str, GUI_TITLE_MAX);
		}
...
static __s32 __playsta_install_hide_timmer(movie_playsta_scene_t *scene_para)
{
	...
	{
		GUI_SetTimer(scene_para->hfrm, scene_para->hide_timmer_id, 500, NULL);//3秒
	}
	else
	{
...
static __s32 __playsta_reset_hide_timmer(movie_playsta_scene_t *scene_para)
{
	...
	if(GUI_IsTimerInstalled(scene_para->hfrm, scene_para->hide_timmer_id))
	{
		GUI_ResetTimer(scene_para->hfrm, scene_para->hide_timmer_id, 500, NULL);//3秒
	}
