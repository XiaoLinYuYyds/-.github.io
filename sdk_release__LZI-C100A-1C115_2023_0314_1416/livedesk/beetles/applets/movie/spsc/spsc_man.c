...
typedef struct
{
	...
	__bool btime_set_mode;//当前是否处于选时模式
}
movie_spsc_scene_t;
...
static __s32 __spsc_ctrl_scene_cmd_proc(movie_spsc_scene_t *sence_para, __s32 msg_id, __s32 dwAddData2)
{
	...
	switch(msg_id)
	{
		...
		case spsc_ctrl_scene_msg_screenset:
		{
			...
			__spsc_ctrl_save_zoom_mode(dwAddData2);
			return EPDK_OK;
		}
...
__bool movie_spsc_scene_get_timeset_mode(void *handle)
{
	movie_spsc_scene_t *scene_para;
	scene_para = (movie_spsc_scene_t *)handle;
	...
	return scene_para->btime_set_mode;
}


H_WIN movie_spsc_scene_get_prog_scene_hwnd(void *handle)
{
	H_WIN hwnd;
	movie_spsc_scene_t *scene_para;
	scene_para = (movie_spsc_scene_t *)handle;
	...
	hwnd = movie_spsc_prog_scene_get_hwnd(scene_para->spsc_prog_scene);
	return hwnd;
}
