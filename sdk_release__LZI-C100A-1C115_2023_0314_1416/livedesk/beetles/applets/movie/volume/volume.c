...
static __s32 __volume_update_volume_ui(movie_volume_scene_t *scene_para)
{
	...
	//画音量图标
	{
		...
		GUI_ClearRect(ui_para->uipara_icon[movie_volume_icon_rvolume].x, ui_para->uipara_icon[movie_volume_icon_rvolume].y, ui_para->uipara_icon[movie_volume_icon_rvolume].x + 20, ui_para->uipara_icon[movie_volume_icon_rvolume].y + 20);
...
static __s32 __volume_install_hide_timmer(movie_volume_scene_t *scene_para)
{
	...
	{
		GUI_SetTimer(scene_para->hfrm, scene_para->hide_timmer_id, 500, NULL);//2秒
	}
	else
	{
...
static __s32 __volume_reset_hide_timmer(movie_volume_scene_t *scene_para)
{
	...
	if(GUI_IsTimerInstalled(scene_para->hfrm, scene_para->hide_timmer_id))
	{
		GUI_ResetTimer(scene_para->hfrm, scene_para->hide_timmer_id, 500, NULL);//2秒
	}
