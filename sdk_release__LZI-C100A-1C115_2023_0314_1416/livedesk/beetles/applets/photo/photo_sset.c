...
static photo_sset_res_id_t sset_red_id[MSET_ITEM_MAX] =
{
	#if SP_APP_PHOTO_BG
	{
		//0, SetBg
		...
		if(i == sset_ctrl->focus_id)
		{
			...
			if((sset_ctrl->main_id == ID_PHOTO_MSET_INTERVAL) && (i == 3))
			{
				if(1 <= sset_ctrl->custom_interval && sset_ctrl->custom_interval <= 2)
				{
					...
					{
						rt_focus.x0 = x + MENU_LIST_STR_X_OFFSET + 20;
					}
					else
					{
						rt_focus.x0 = x + MENU_LIST_STR_X_OFFSET + PHOTO_FONT_BYTE_W  + 20;
					}
				...
				GUI_SetColor(sset_ctrl->fcs_color);
				GUI_DispStringAt(buf, x + MENU_LIST_STR_X_OFFSET + 20, y + MENU_LIST_STR_Y_OFFSET);
				eLIBs_strcpy(long_str, buf);
			}
			else
			{
				GUI_DispStringAt(sset_res->string[i], x + MENU_LIST_STR_X_OFFSET + 20, y + MENU_LIST_STR_Y_OFFSET);
