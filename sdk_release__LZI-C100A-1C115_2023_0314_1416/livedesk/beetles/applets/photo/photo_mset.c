	...
	switch(mode)
	{
		case PAINT_PHOTO_MSET_ALL:
		{
...
#endif
			//eLIBs_printf("photo_uipara->sset_main_layer.w=%d\n", photo_uipara->sset_main_layer.w);
			//eLIBs_printf("photo_uipara->sset_main_layer.h=%d\n\n", photo_uipara->sset_main_layer.h);
			...
			for(i = 0; i < MSET_ITEM_PER_PAGE; i++)
			{
				...
				//eLIBs_printf("gui_rect.x0=%d\n", gui_rect.x0);
				//eLIBs_printf("gui_rect.y0=%d\n", gui_rect.y0);
				//eLIBs_printf("gui_rect.x1=%d\n", gui_rect.x1);
				//eLIBs_printf("gui_rect.y1=%d\n\n", gui_rect.y1);
...
#else
					if(msg->dwAddData1 == GUI_MSG_KEY_LONGNUM1 || msg->dwAddData1 == GUI_MSG_KEY_LONGNUM2)
#endif
					{
						//向下传做处理(放大时上下长按)
					}
	...
	{
		{
			...
			if(mset_ctrl && mset_ctrl->lr_adjust_sset == 1)
			{
				if((msg->dwAddData1 == GUI_MSG_KEY_NUM1) || (msg->dwAddData1 == GUI_MSG_KEY_LONGNUM1))
				{
					if(ANOLE_GetSta() == ANOLE_MODE_ZOOM)
					{
						msg->dwAddData1 = GUI_MSG_KEY_DOWN;
				...
				else if((msg->dwAddData1 == GUI_MSG_KEY_NUM2) || (msg->dwAddData1 == GUI_MSG_KEY_LONGNUM2))
				{
					if(ANOLE_GetSta() == ANOLE_MODE_ZOOM)
					{
						msg->dwAddData1 = GUI_MSG_KEY_UP;
...
H_WIN photo_mset_win_create(H_WIN h_parent, photo_mset_para_t *para)
{
	...
	//eLIBs_printf("fb.size.width=%d\n", fb.size.width);
	//eLIBs_printf("fb.size.height=%d\n\n", fb.size.height);
