...
static __s32 sset_win_create(__gui_msg_t *msg, __s32 mset_id)
{
	...
	switch(mset_id)
	{
		...
		case ID_PHOTO_MSET_BGMUSIC:
			rect.x = photo_uipara->sset_bg_muisc_layer.x + 256;
			rect.y = photo_uipara->sset_bg_muisc_layer.y + 215;
		...
		case ID_PHOTO_MSET_ZOOM:
			rect.x = photo_uipara->sset_zoom_layer.x - 41;
			rect.y = photo_uipara->sset_zoom_layer.y + 227;
		...
		case ID_PHOTO_MSET_ROTATE:
			rect.x = photo_uipara->sset_rotate_layer.x - 63;
			rect.y = photo_uipara->sset_rotate_layer.y + 343;
		...
		case ID_PHOTO_MSET_BRIGHT:
			rect.x = photo_uipara->sset_brightness_layer.x - 39;
			rect.y = photo_uipara->sset_brightness_layer.y + 213;
			rect.width = photo_uipara->sset_brightness_layer.w;
			rect.height = photo_uipara->sset_brightness_layer.h;
			sset_para.value = (get_sys_bright()+1) * 9 / CONST_DispBright_Max;
			break;

		case ID_PHOTO_MSET_SLIDESHOW:
			rect.x = photo_uipara->sset_slideshow_layer.x + 158;
			rect.y = photo_uipara->sset_slideshow_layer.y + 215;
		...
		{
			__s32 temp;
			rect.x = photo_uipara->sset_interval_layer.x + 63;
			rect.y = photo_uipara->sset_interval_layer.y + 215;
		...
		case ID_PHOTO_MSET_SCALE:
			rect.x = photo_uipara->sset_scale_layer.x + 63;
			rect.y = photo_uipara->sset_scale_layer.y + 215;
...
static __s32 photo_sset_cmd_proc(__gui_msg_t  *msg)
{
	...
	switch(HIWORD(msg->dwAddData1))
	{
		...
		{
			__s32 bright;
			bright = (msg->dwAddData2 * CONST_DispBright_Max) / 9;
			dsk_set_display_global(bright, DISP_CMD_SET_BRIGHT);
			break;
		}
...
static __s32 _app_photo_Proc(__gui_msg_t  *msg)
{
	...
	switch(msg->id)
	{
		case GUI_MSG_CREATE:
		{
			...
			photo_init(msg);
			__wrn("_app_photo_Proc GUI_MSG_CREATE \n");
			//spsc_win_create(msg);
			return 0;
		}
			...
			else
			{
				if(msg->dwAddData1 == GUI_MSG_KEY_MENU)
				{
					...
					msg->dwAddData1 = GUI_MSG_KEY_ESCAPE;
				        ...
				        || GUI_MSG_KEY_SUBMENU == msg->dwAddData1)
				{
					DEBUG_photo("\n _app_ GUI_MSG_KEY_LONGMENU -> GUI_MSG_KEY_MENU ____ \n");
					msg->dwAddData1 = GUI_MSG_KEY_MENU;
				}
				else if(0) //(msg->dwAddData1 == GUI_MSG_KEY_VADD)
				{
					DEBUG_photo("\n _app_ GUI_MSG_KEY_VADD -> GUI_MSG_KEY_UP ____ \n");
					msg->dwAddData1 = GUI_MSG_KEY_UP;
				}
				else if(0) //(msg->dwAddData1 == GUI_MSG_KEY_LONGVADD)
				{
...
static __s32 app_photo_notify_delete_sub_scene2(H_WIN hmanwin)
{
	/*
