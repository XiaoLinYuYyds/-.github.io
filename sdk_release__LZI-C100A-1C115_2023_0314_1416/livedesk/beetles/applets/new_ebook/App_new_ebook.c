...
static __s32 new_ebook_GetListItemFileFullPath(app_new_ebook_ctrl_t *list_para, __s32 ItemIndex, char *FilePath)
{
	...
	ret = rat_get_item_info_by_index(list_para->rat.rat_handle, ItemIndex, &ItemInfo);//获取当前项目信息索引,文件索引id不能大于该媒体文件类型的文件总数
	#if 1//参数 ItemInfo是否有数据,路径、电子书格式
		__wrn("iteminfo.path = %s, iteminfo.Format = %d\n",ItemInfo.Path, ItemInfo.Format);
	#endif
	//ret = rat_npl_index2file(list_para->rat.rat_handle, ItemIndex, path);
	...
	//eLIBs_strcpy(FilePath, path);			//不去操作rat 内存
	return EPDK_OK;
}
...
/************************************************************************************
*Description	:		通过文件索引id播放文件
************************************************************************************/
static __s32 PlayEbookFiles(__gui_msg_t *msg, char *FileName)
{
	app_new_ebook_ctrl_t *this;
	HRATNPL h_rat;
	char file_path[1024];
	...
	/*h_rat = rat_npl_open(RAT_MEDIA_TYPE_EBOOK);			//打开RAT播放列表模块的对应媒体类型文件
	__wrn("h_rat = %d\n",h_rat);
	if(!h_rat){
		__wrn("h_rat is null...\n");
	}
	//rat_npl_set_cur(this->rat.rat_handle, index);//设置当前要播放的文件索引id
	//index = rat_npl_get_cur(this->rat.rat_handle);
	eLIBs_memset(file_path, 0, sizeof(file_path));
	rat_npl_set_cur(h_rat, index);						//设置当前要播放的文件索引id
	index = rat_npl_get_cur(h_rat);						//获取RAT播放列表当前要播放的文件索引id
	__wrn("\nindex = %d\n",index);
	rat_npl_index2file(h_rat, 1, file_path);			//通过文件索引id获取文件名
	eLIBs_memcpy(FileName, &file_path, 1024);	//拷贝文件路径到发文件名变量
	__wrn("file_path = %s...\n",file_path);
	rat_npl_close(h_rat);								//关闭RAT播放列表模块*/
}
...
static __s32 app_new_ebook_decode_create(__gui_msg_t *msg)
{
	...
	__s32 	ret;
	__u8    err;
	__u32	index = -1;//传入到电子书解码初始化函数的文件索引id
	...
	/* 创建 mbook core 句柄 */
	err = 0;
	if(new_ebook_ctrl->index != -1){//如果文件索引id 不为-1时，进入获取传入的文件索引id
		index = new_ebook_ctrl->index;
		__wrn("decode file index = %d...\n",index);
	}
	new_ebook_ctrl->mbook = EBOOK_Decode_Init(NULL, &err, index);//电子书解码初始化
...
static __s32 __app_new_ebook_proc(__gui_msg_t *msg)
{
	switch(msg->id){
		case GUI_MSG_CREATE:{/*0*/
			...
			//dsk_reg_get_para_by_app(REG_APP_EBOOK);
			com_set_palette_by_id(ID_EBOOK_WKM_BMP);		//调色板id
			{
				...
				PlayEbookFiles(msg, FileName);//通过传入的文件索引播放文件
				__wrn("\nFileName = %s\n",FileName);
			#endif
		...
		case GUI_MSG_CLOSE:{/*3*/
			...
			app_new_ebook_cmd2para(msg->h_deswin, NEW_SWITCH_TO_OTHER_APP, NEW_EBOOK_SW_TO_NEW_EXPLORER, 2);//退出到新的资源管理器列表
