...
static __s32 fm_mainfrw_key_proc(__gui_msg_t *msg)
{
	...
	if(msg->dwAddData2 == KEY_UP_ACTION)
	{
		switch(lastkey)
		{
			...
			case VK_ENTER:
			case GUI_MSG_KEY_FM:
				SEND_MSG(GUI_MSG_COMMAND, msg->h_deswin, FM_GetManWnd(),
		...
		switch((msg->dwAddData1))
		{
			case GUI_MSG_KEY_LONGFM:
				...
				//esKRNL_TimeDly(200);
				//SEND_MSG(DSK_MSG_SCREEN_CLOSE, msg->h_deswin, GUI_WinGetHandFromName("init"), 0, 0, 0);
				break;
	...
	else
	{
		switch(msg->dwAddData1)
		{
			...
			case GUI_MSG_KEY_NUM9:
				SEND_MSG(GUI_MSG_COMMAND, msg->h_deswin, FM_GetManWnd(),
				         MAKELONG(FM_MAINWND_ID, ID_FM_MAINWND_CLICKUP), msg->dwAddData1, msg->dwReserved);
				break;
		}
