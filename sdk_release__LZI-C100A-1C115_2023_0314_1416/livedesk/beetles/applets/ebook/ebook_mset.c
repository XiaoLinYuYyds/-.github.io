...
#define MSET_ITEM_START_X   120
#define	MSET_ITEM_BMP_OFFSET  200//菜单中每个item  间图片的间距
#define   MSET_ITEM_TXT_OFFSET   180  //菜单中每个item间字符的间距
enum
{
...
static __s32 ebook_mset_paint(__gui_msg_t *msg, __s8 mode)
{
	...
	//x = MSET_ITEM_BMP_OFFSET;//55; //调整菜单图标的位置
	x = ebook_uipara->sset_bgmusic_lyr.x + 55 - 25; //调整菜单图标的位置
	...
	switch(mode)
	{
		case PAINT_EBOOK_MSET_ALL:
		{
			...
			for(i = 0; i < MSET_ITEM_MAX; i++)
			{
				if(i == mset_ctrl->focus_id)
				{
					...
					gui_rect.x0 =  x + i * (ebook_uipara->bmp_mset_item.w + MSET_ITEM_BMP_OFFSET) - MSET_ITEM_TXT_OFFSET / 2; //菜单文字的位置
		...
		case PAINT_EBOOK_MSET_ITEM:
		{
			...
			gui_rect.x0 = x + mset_ctrl->focus_id * (ebook_uipara->bmp_mset_item.w + MSET_ITEM_BMP_OFFSET) - MSET_ITEM_TXT_OFFSET / 2; //菜单文字的位置
			gui_rect.y0 = ebook_uipara->bmp_mset_item.h - 10;
			gui_rect.x1 = gui_rect.x0 + ebook_uipara->bmp_mset_item.w + MSET_ITEM_TXT_OFFSET ;
			gui_rect.y1 = ebook_uipara->bmp_mset_bg.h;
			GUI_SetColor(mset_ctrl->fcs_color);
			...
			gui_rect.x0 = x + mset_ctrl->old_focus * (ebook_uipara->bmp_mset_item.w + MSET_ITEM_BMP_OFFSET) - MSET_ITEM_TXT_OFFSET / 2; //菜单文字的位置
			gui_rect.y0 = ebook_uipara->bmp_mset_item.h - 10 ;
			gui_rect.x1 = gui_rect.x0 + ebook_uipara->bmp_mset_item.w + MSET_ITEM_TXT_OFFSET ;
			gui_rect.y1 = ebook_uipara->bmp_mset_bg.h;
			GUI_SetColor(mset_ctrl->unfcs_color);
