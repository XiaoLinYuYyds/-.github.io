...
static void __decode_get_charset(__mbook_decode_t *hdle)
{
	...
	if((buff[0] == 0xff) && (buff[1] == 0xfe))		//获取文件流前两个字节，分别为0xff、0xfe表示UTF16LE编码格式文件
	{
		p_decode->charset = EPDK_CHARSET_ENM_UTF16LE;
	}
	else if((buff[0] == 0xfe) && (buff[1] == 0xff))//文件流前两个字节，分别为0xfe、0xff表示UTF16BE编码格式文件
	{
		p_decode->charset = EPDK_CHARSET_ENM_UTF16BE;
	}
	else if((buff[0] == 0xef) && (buff[1] == 0xbb) && (buff[2] == 0xbf))//UTF8有BOM编码格式
	{
...
__s32   MBOOK_Decode_Config(H_DECODE hdle, __decode_config_t *config)
{
	...
	if(config->show_rotate == 0)
	{
		...
		analysis_cfg.page_row   = (config->show_height - config->bottom_width) / (config->font_size + config->row_space);//页的最大行数
