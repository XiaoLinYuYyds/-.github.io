...
__s32 app_explorer_create(root_para_t  *para)
{
	...
	switch(para->data)
	{
		...
		case ID_EXPLORER_MOVIEFAVOR:
		{
			explr_ctrl->view_mode = EXPLR_LIST_MODE;
			explr_ctrl->media_type = RAT_MEDIA_TYPE_FAVOR_VIDEO;
			explr_ctrl->root_type = para->root_type;
			explr_ctrl->operate_media = 1;
			__wrn("come here\n");
		}
		break;
#endif
		...
		case ID_EXPLORER_FIRMWARE:
			__here__;
			explr_ctrl->view_mode = EXPLR_LIST_MODE;
			explr_ctrl->media_type = RAT_MEDIA_TYPE_FIRMWARE;
			explr_ctrl->root_type = para->root_type;
			break;
...
static __s32 app_explorer_on_destroy(H_WIN h_exp_win)
{
	...
	if(explr_ctrl->h_dialoag_win)
	{
		__msg("h_dialoag_win is not null, delete it!\n");
		app_dialog_destroy(explr_ctrl->h_dialoag_win) ;
		explr_ctrl->h_dialoag_win = NULL ;
	}
	...
	__msg("explr_ctrl->h_dialoag_win = %x\n", explr_ctrl->h_dialoag_win);
...
static __s32 _explorer_manager_win_cb(__gui_msg_t *msg)
{
	switch(msg->id)
	{
		...
		case GUI_MSG_DESTROY:
			...
			//cene_bgd_set_state(BGD_STATUS_SHOW);
...
static H_LYR explorer_8bpp_layer_create(void)
{
	...
	fb.fmt.fmt.rgb.pixelfmt =  PIXEL_COLOR_ARGB8888;//PIXEL_MONO_8BPP;
	lstlyr.src_win.x  		= 0;
	lstlyr.src_win.y  		= 0;
...
static __s32 explorer_draw_bg(explorer_ctrl_t	*explr_ctrl)
{
...

	{
		...
		*/
		...
		{
			ES_FILE *fp;
			fp = eLIBs_fopen("b:\\disp\\display", "r+");//解决TVOUT输出时列表闪烁的问题。
...
static __s32 explorer_scene_create(__gui_msg_t *msg)
{
...
#if SP_FavoriteFiles
	ListPara.operate_media = explr_ctrl->operate_media;
#endif
	...
	explr_ctrl->BG_layer = explorer_8bpp_layer_create(); //现在修改为32bpp的了
	...
	GUI_LyrWinSetTop(explr_ctrl->list_layer);						//set picture layer as top level
