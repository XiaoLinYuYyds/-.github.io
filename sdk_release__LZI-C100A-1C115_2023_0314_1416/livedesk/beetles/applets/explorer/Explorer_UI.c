...
#ifndef ID_EXPLORER_USB_BMP
#define ID_EXPLORER_USB_BMP		ID_EXPLORER_FOLDER_BMP
#endif

#ifndef ID_EXPLORER_SD_BMP
#define ID_EXPLORER_SD_BMP		ID_EXPLORER_FOLDER_BMP
#endif
...
static explorer_viewer_ui_t  explr_viewer_ui_400_240 =
{
	...
	{ 431,	8,	7,	224},  //{ 391,	8,	7,	224},          //scroll bg rect
	{ 433,	22,	3,	194},   //{ 393,	22,	3,	194},   		 //scroll rect
	...
	{
		{ 156, 8, 254, 210},//{ 136, 8, 254, 210},				//item rect
...
static explorer_viewer_ui_t  explr_viewer_ui_800_480 =
{
	...
	//menuview attribute
	{ 0, 0, 640, 480 },             //menuview(frame window)
	{ 0, 0, 640, 480 },             //browser bg
	{ 0, 0, 640, 20  },              //browser bg top
	{ 0, 0, 0, 	0 },                 //browser bg bottom
	{ 0, 0, 0,  0 },              //browser bg left

	{ 620,	10,	7,	450}, //{ 391,	8,	7,	224},          //scroll bg rect文件浏览器列表滚动条背景区域
	{ 623,	24,	5,	419}, //{ 393,	22,	3,	194},   		 //scroll rect文件浏览器列表滚动条图片区域
	9, 	4, 	9,                        //scroll head/body/tail height

	{ 0, 15, 640, 450},        //(the whole listbar rectangle)

	//list item attribute，以下坐标为点对于listbar rectangle的相对坐标
	{
#if SP_FavoriteFiles
		{ 156, 8, 436, 455},//{ 136, 8, 254, 210},				//item rect文件名区域
		{ 16, 0, 436, 35 },       			//item rect单行文件名区域
		{ 16 , 0, 436, 2 },        			//item_top_line;
		{ 16 , 0, 436, 2 },        			//item_bottom_line;
		{ 16 , 2, 2, 38},       				//item_dividing_line;
#else
		{ 156, 8, 420, 455},//{ 136, 8, 254, 210},				//item rect文件名区域
		{ 0, 0, 420, 35 },       			//item rect单行文件名区域
		{ 0 , 0, 420, 2 },        			//item_top_line;
		{ 0 , 0, 420, 2 },        			//item_bottom_line;
		{ 0 , 2, 2, 38},       				//item_dividing_line;
#endif
#if SP_FavoriteFiles
		{ 40, 4, 15, 15 },        			//icon rect
#else
		{ 24, 4, 15, 15 },        			//icon rect
#endif
		{ 0, 0, 50, 30 },       			//miniature rect, 缩略图 UI 参数
...
#if SP_FavoriteFiles
		{ 75, 0, 338, 30 },  //text rect文件名显示范围

		{ 23, 2, 254, 30 },      			//focus icon rect(select picture)
#else
		{ 59, 0, 338, 30 },  //text rect文件名显示范围

		{ 7, 2, 254, 30 },      			//focus icon rect(select picture)
#endif
		{ 24, 53, 88, 110},					//媒体类型图标位置(显示在左边区域的大图标)

		{0, 12, 120, 20 },      			//title rect, only for file manager文件管理器标题
		{0, 164 + 240, 130, 20},      		//file size rect文件大小
		{0, 184 + 240, 130, 20},      		//file create time rect文件创建时间
		{0, 0 + 240, 150, 34 },      				//number rect日期
	},
...
explorer_viewer_ui_t *explorer_get_viewer_ui_param(void)
{
	...
	explr_viewer_ui_800_480.layer.scn_rect.x = (screen_width - 640) / 2;
	explr_viewer_ui_800_480.layer.scn_rect.y = (screen_height - 480) / 2;
	__wrn("explr_viewer_ui_800_480.layer.scn_rect.x=%d\n",
	      explr_viewer_ui_800_480.layer.scn_rect.x);
	__wrn("explr_viewer_ui_800_480.layer.scn_rect.y=%d\n",
	      explr_viewer_ui_800_480.layer.scn_rect.y);
...
//获取 列表条目 显示的 页数： 列表模式或正方形模式分别画一个列表项目条矩形显示区域
...
*
*Description	:	  获取列表视图的icon图标 然后申请使用图片资源
*
*Arguments  	:
*
*Return     	:
...
*
*Description	:	  释放列表视图的icon图标 然后释放申请使用图片资源
*
*Arguments  	:
*
*Return     	:
...
//获取文件类型项目的icon图标，用于不同文件类型的正常显示和选中显示图标
void *explorer_get_file_item_icon(explorer_view_mode_e view_mode, rat_media_type_t media_type, exp_icon_status_e status)
{
	...
	switch(media_type)
	{
		case RAT_MEDIA_TYPE_VIDEO:
#if SP_FavoriteFiles
		case RAT_MEDIA_TYPE_FAVOR_VIDEO://视频文件类型
#endif
			if(status == FOCUSED)
			{
		...
		case RAT_MEDIA_TYPE_PIC://照片文件类型
			if(status == FOCUSED)
			{
		...
		case RAT_MEDIA_TYPE_AUDIO://音频文件类型
			if(status == FOCUSED)
			{
		...
		case RAT_MEDIA_TYPE_EBOOK://电子书文件类型
			if(status == FOCUSED)
			{
		...
		case RAT_MEDIA_TYPE_FOLDER://文件夹目录
			if(status == FOCUSED)
			{
		...
		case RAT_MEDIA_TYPE_USB_DEVICE://USB设备类型
			if(status == FOCUSED)
			{
		...
		case RAT_MEDIA_TYPE_SD_DEVICE://SD卡设备类型
			if(status == FOCUSED)
			{
