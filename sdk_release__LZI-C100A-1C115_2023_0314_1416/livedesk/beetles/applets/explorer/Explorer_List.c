...
#define TIMEOUT_ID_TIMER_AutoUpdateFirmware		8

#if TIMEOUT_ID_TIMER_AutoUpdateFirmware
#define ID_TIMER_AutoUpdateFirmware		0x1000
#endif

static   __u32 ExplorerTimerId = 0x16;
#define	 C_EXP_TIMER_INTERVAL	300				// 2second

static   __u32 ExplorerPlayTimeId = 0x17;
#define  C_EXP_START_PLAY_TIME   400

static char explore_key_count = 0;
static char explore_key_count_t = 0;
static __u32 root_type_explore = 10;
...
__s32 File_manager_play_file(explr_list_para_t *list_para)
{
	...
	{
		if(list_para->operate_media == 1)
		{
			media_type = RAT_MEDIA_TYPE_FAVOR_VIDEO;
		}
		else
		{
	...
	list_para->rat.handle = rat_open_ex(root_type , media_type , 0);	//全部搜索
...
static __s32  file_manager_set_current_file_for_play(__gui_msg_t *msg)
{
	...
	if(media_type == RAT_MEDIA_TYPE_VIDEO || media_type == RAT_MEDIA_TYPE_FAVOR_VIDEO)
	{
		list_para->root_para->enter_media_type[m_eMediaTypeList_video] = RAT_MEDIA_TYPE_VIDEO;
	}
	else if(media_type == RAT_MEDIA_TYPE_AUDIO || media_type == RAT_MEDIA_TYPE_FAVOR_AUDIO)
	{
		list_para->root_para->enter_media_type[m_eMediaTypeList_audio] = RAT_MEDIA_TYPE_AUDIO;
	}
	...
	switch(media_type)
	{
		...
		case RAT_MEDIA_TYPE_FAVOR_VIDEO:
			explorer_cmd2parent(msg->h_deswin, SWITCH_TO_OTHER_APP, EXPLR_SW_TO_MOVIE , 1);
			break;
#endif
...
static void Draw_Explore_Directory(__u32 focus, GUI_FONT *font)

{
	...
	__u32 start_x = 20;
	__u32 start_y = 230 + 200;
	GUI_RECT gui_rect;
	char item_str[1][128];
	__s32 lang_id = STRING_EXPLR_DIRECTORY;
	...
	if(focus == 1)
	{
		directory_fcous = theme_open(ID_EXPLORER_SD_MOVIE_FOCUSED_BMP);
		directory_unfcous = theme_open(ID_EXPLORER_USB_MOVIE_UNFOCUSED_BMP);
		root_type_explore = 1;
	}
	else if(focus == 0)
	{
		directory_fcous = theme_open(ID_EXPLORER_USB_MOVIE_FOCUSED_BMP);
		directory_unfcous = theme_open(ID_EXPLORER_SD_MOVIE_UNFOCUSED_BMP);
		root_type_explore = 0;
	}
	...
	if(focus == 1)
	{
		GUI_BMP_Draw(fbmp, start_x, start_y);
		GUI_BMP_Draw(ubmp, start_x + 50, start_y);
	}
	else
	{
		GUI_BMP_Draw(ubmp, start_x, start_y);
		GUI_BMP_Draw(fbmp, start_x + 50, start_y);
	}

	dsk_langres_get_menu_text(lang_id, item_str, GUI_TITLE_MAX);// 显示播放目录
	GUI_DispStringInRect((const char *)item_str, &gui_rect, GUI_TA_VCENTER | GUI_TA_LEFT);
	dsk_theme_close(directory_fcous);
	dsk_theme_close(directory_unfcous);
}

#if 0	//SP_FavoriteFiles
static void Draw_Explore_Favor_list_icon(void)
{
	__s32 favor_list = 0;
	void *bmp;
	GUI_RECT gui_rect;
	__u32 start_x = 700;	// 坐標需調整
	__u32 start_y = 160;
	gui_rect.x0 = 700;
	gui_rect.y0 = 160;
	...
	GUI_ClearRectEx(&gui_rect);//清除区域
	favor_list = theme_open(ID_EXPLORER_FAVOR_PLAYLIST_BMP);
	bmp = theme_hdl2buf(favor_list);
	GUI_BMP_Draw(bmp, start_x, start_y);
	dsk_theme_close(favor_list);
}
#endif
...
H_WIN explorer_list_win_create(H_WIN h_parent, explr_list_para_t *para)
{
	...
	explr_list_para_t *list_para;
	FB fb;
	#if 0	//检测framewin窗口是否内存泄漏-->查看打印的Pages free的大小反复切换manwin界面是否会恢复
...
#if SP_FavoriteFiles
	list_para->operate_media = para->operate_media;
#endif
	ExplorerListWinGetSearchPath(list_para);
...
static __s32 explorer_listview_onpaint(H_WIN  list_win)
{
	...
	__wrn("explorer_listview_onpaint...\n");

	if(GUI_LyrWinGetSta(list_para->list_layer) != GUI_LYRWIN_STA_ON)
	{
		__wrn("list layer not on, do not paint...\n");
		return EPDK_OK;
	}
	...
	else
	{
		...
		if(list_para->root_type == RAT_TF)//显示目录
		{
			Draw_Explore_Directory(1, list_para->font);
		}
		else if(list_para->root_type == RAT_USB)
		{
			Draw_Explore_Directory(0, list_para->font);
		}
...
static __s32  explorer_list_win_on_destroy(__gui_msg_t *msg)
{
	...
	if(GUI_IsTimerInstalled(list_para->list_win, ExplorerPlayTimeId))
	{
		__msg("kill the ExplorerPlayTimeId timer\n");
		GUI_KillTimer(list_para->list_win, ExplorerPlayTimeId);
	}

	if(list_para->h_fw_update_dlg)
	{
	...
	if(list_para->media_type == RAT_MEDIA_TYPE_VIDEO)
	{
		list_para->root_para->enter_media_type[m_eMediaTypeList_video] = RAT_MEDIA_TYPE_VIDEO;
	}
	else
	{
		list_para->root_para->enter_media_type[m_eMediaTypeList_video] = RAT_MEDIA_TYPE_FAVOR_VIDEO;
	}
	...
	*/
	{
		...
		__wrn("listbar start id = %d\n", ListBarScene.start_id);
		__wrn("listbar focus id = %d\n", ListBarScene.focus_id);
...
__s32 GetFirmwareInfo_FileSize(void)
{
...
__s32 CheckFileInfo_FileSize(const char *filePath, __s32 fileSize)
{
	__s64	fileSize_byte;
...
#define PATH__AUTOEXE_LZ_C100_IMG_INI		PATH__LZ_C100_IMG_FOLDER"\\updatefirmware_autoexe.lz"
#else
#define PATH__AUTOEXE_LZ_C100_IMG_INI		"x:\\updatefirmware_autoexe.lz"
#endif
__s32 Explorer_OpenFirmwareFile(explr_list_para_t *list_para)
{
	char path[RAT_MAX_FULL_PATH_LEN] = "";
	__s32 ret;
	ret = GetListItemFileFullPath(list_para, list_para->rat.index, path);

	if(EPDK_OK == ret)
	{
		__msg("fw_update_file:<%s>\n", path);
		eLIBs_memset(list_para->fw_update_file, 0, sizeof(list_para->fw_update_file));
		eLIBs_strcpy(list_para->fw_update_file, path);

		if(CheckFileInfo_FileSize(path, list_para->firmwareSize) == EPDK_OK)
		{
			ES_FILE *pEsFile;
			char strPath_autoexe[] = PATH__AUTOEXE_LZ_C100_IMG_INI;
			strPath_autoexe[0] = path[0];
			pEsFile = eLIBs_fopen(strPath_autoexe, "rb");

			if(pEsFile)
			{
				eLIBs_fclose(pEsFile);
...
static __s32  explorer_list_win_on_enter_key(__gui_msg_t *msg)
{
	...
	__wrn("list_para->rat.index:%d\n", list_para->rat.index);/*播放文件的索引*/
	switch(list_para->media_type)
	{
		...
		case RAT_MEDIA_TYPE_FAVOR_VIDEO:
		{
			list_para->root_para->favor_index = list_para->rat.index;
			__wrn("list_para->rat.index:%d\n", list_para->rat.index);
			explorer_cmd2parent(msg->h_deswin, SWITCH_TO_OTHER_APP, EXPLR_SW_TO_MOVIE, 1);
			break;
		}
	...
	if((KEY_DOWN_ACTION == msg->dwAddData2) || (KEY_REPEAT_ACTION == msg->dwAddData2))
	{
		switch(msg->dwAddData1)
		{
			...
			case GUI_MSG_KEY_ENTER:
				...
				else if(list_para->media_type == RAT_MEDIA_TYPE_FIRMWARE)
				{
					Explorer_OpenFirmwareFile(list_para);
				}
				else
				{
					explorer_list_long_string_stop_roll(list_para);
			...
			{
				__msg("GUI_MSG_KEY_LONGMENU back to up\n");

				if(list_para->media_type != RAT_MEDIA_TYPE_ALL)
				{
					explorer_cmd2parent(msg->h_deswin, SWITCH_TO_EXPLORER, 0 , 0);
					return EPDK_OK;
				}

				explore_key_count = 3;

				if(explore_key_count >= 3)		//回到上一级菜单.
				{
					explore_key_count = 0;

					if(NULL == list_para->h_dialog)
					{
						if(list_para->cur_file_list == NULL)
						{
							return EPDK_OK;
			...
			case GUI_MSG_KEY_FAVORFILE:
			{
				if(last_key == GUI_MSG_KEY_NUM1 || msg->dwAddData1 == GUI_MSG_KEY_FAVORFILE)
				{
					if(list_para->media_type == RAT_MEDIA_TYPE_VIDEO)
					{
						explorer_favoriefile__add_or_del(msg);
					}
				}
			...
			case GUI_MSG_KEY_FAVORPLAY:
			{
				if(last_key == GUI_MSG_KEY_NUM0 || msg->dwAddData1 == GUI_MSG_KEY_FAVORPLAY)
				{
					...
					if(list_para->media_type == RAT_MEDIA_TYPE_VIDEO ||
					    list_para->media_type == RAT_MEDIA_TYPE_FAVOR_VIDEO)
					{
						ret = explorer_shift_playlist(msg);
						...
						if(ret == EPDK_OK)
						{
							if(list_para->operate_media == 0)
							{
								explorer_cmd2parent(msg->h_deswin, SWITCH_TO_OTHER_APP, EXPLR_SW_TO_FAVOR_EXPLR, 1);
							}
							else
							{
								explorer_cmd2parent(msg->h_deswin, SWITCH_TO_OTHER_APP, EXPLR_SW_TO_FAVOR_EXPLR, 0);
							}
						}
					}
				}
...
static __s32 draw_listview_item_text(__lbar_draw_para_t *draw_param)
{
	...
	__wrn("draw_listview_item_text  FileName = %s\n",FileName);
	if(draw_param->mode == LBAR_MODE_FOCUS)
	{
...
#else
			media_type = list_para->media_type;
#endif
		}
		else
		{
...
*
*Description	:	 ListBar绘制消息处理函数，列表条目项目绘制
*
*Arguments  	:
*
*Return     	:
...
*/
#if SP_FavoriteFiles
static __s32 explorer_shift_playlist(__gui_msg_t *msg)
{
	H_WIN	 h_dialog = NULL;
	explr_list_para_t *list_para;
	HRAT hrat;
	...
	list_para = (explr_list_para_t *)GUI_WinGetAttr(msg->h_deswin);
	__wrn("list_para->media_type:%d\n", list_para->media_type);

	if(list_para->media_type == RAT_MEDIA_TYPE_VIDEO)
	{
		if(1)
		{
			__s32 lang_id[] = {STRING_MOVIE_TIPS, STRING_MOVIE_FAVOR_LIST};
			__wrn("-------default_dialog--------\n");
		...
		hrat = rat_open_ex(list_para->root_type, RAT_MEDIA_TYPE_FAVOR_VIDEO, 0);
		...
		if(count == 0)
		{
			__s32 lang_id[] = {STRING_MOVIE_TIPS, STRING_MOVIE_FAVOR_NO_MOVIE};
			__wrn("-------default_dialog--------\n");
			default_dialog(h_dialog, hDstWin, APP_STORE_UP_PLAY_DLG_ID, ADLG_OK, lang_id);
			esKRNL_TimeDly(100);
	...
	{
		__wrn("-------stro_up_play_flag = 0--------\n");

		if(1)
		{
			__s32 lang_id[] = {STRING_MOVIE_TIPS, STRING_MOVIE_FAVOR_NORMAL_LIST};
			__wrn("-------default_dialog--------\n");
			default_dialog(h_dialog, hDstWin, APP_STORE_UP_PLAY_DLG_ID, ADLG_OK, lang_id);
			esKRNL_TimeDly(100);
...
//收藏添加或删除
static __s32 explorer_favoriefile__add_or_del(__gui_msg_t *msg)
{
	explr_list_para_t *list_para;
	...
	__wrn("listbar get scene focusid = %d, start id = %d\n", scene.focus_id, scene.start_id);
	{
		...
		ret = rat_get_item_info_by_index(list_para->rat.handle, scene.focus_id, &ItemInfo);
...
static __s32 draw_movie_list_stroe_up_icon(__lbar_draw_para_t *draw_param)
{
	int ret;
	char fileName[RAT_MAX_FULL_PATH_LEN];
	RECT Rect;
	explr_list_para_t *list_para;
	explorer_viewer_ui_t *ui_param;
	void *Favoite_pic_buf;
	__s32 Favoite_pic = 0;
	__s32 picX, picY;
	HRAT hrat = NULL;
	__s32 favoite_picX, favoite_picY;
	explorer_get_item_focus_icon_rect(list_para, &Rect);
	list_para = (explr_list_para_t *)draw_param->attr;
	...
	eLIBs_memset(fileName, 0, sizeof(fileName));
	__here__;
	ret = MusicGetListItemFileFullPath(list_para->rat.handle, draw_param->index, fileName);
	...
	//DebugPrintf("-------------------draw_param->index=%d...\n", draw_param->index);
	{
		...
		if(ret == 1)
		{
			...
			list_para->is_stroe_up_flag[draw_param->index] = 1;
		}
		else
		{
			//DebugPrintf("is_not_favor_file...\n");
			list_para->is_stroe_up_flag[draw_param->index] = 0;
			//i++;
		}
	...
	if(list_para->is_stroe_up_flag[draw_param->index] == 1)
	{
		list_para->stroe_up_flag = 1;
		//DebugPrintf("*\n");
	}
	else
	{
		list_para->stroe_up_flag = 0;
		//DebugPrintf("*\n");
	}
	...
	if(list_para->stroe_up_flag == 1)
	{
		//Favoite_pic_buf = music_get_icon_res(ID_MUSIC_FAVOLITE_SELECT);
		Favoite_pic = theme_open(ID_MUSIC_FAVOLITE_FOCUSED_BMP);
		Favoite_pic_buf = theme_hdl2buf(Favoite_pic);
		favoite_picX = draw_param->rect.x ;
		favoite_picY = draw_param->rect.y + Rect.y + 2;
		GUI_BMP_Draw(Favoite_pic_buf, favoite_picX, favoite_picY);
		//DebugPrintf("*\n");
		dsk_theme_close(Favoite_pic);
	}
...
static __s32 _explorer_listbar_item_draw(__lbar_draw_para_t *draw_param)
{
	...
	__wrn("_explorer_listbar_item_draw  draw_param->mode = %d\n",draw_param->mode);
	switch(draw_param->mode)
	{
		case LBAR_MODE_NORMAL://画列表条目正常模式
			__msg("\n~~~~draw listbar in normal mode!~~~~");
			draw_listview_item(draw_param);
#if SP_FavoriteFiles

			if(list_para->media_type == RAT_MEDIA_TYPE_FAVOR_VIDEO ||
			    list_para->media_type == RAT_MEDIA_TYPE_VIDEO)
			{
				draw_movie_list_stroe_up_icon(draw_param);
			}
...
static __s32 explorer_rat_init(explr_list_para_t *list_para)
{
	...
	*/
	...
	{
		if(list_para->media_type == RAT_MEDIA_TYPE_FIRMWARE)
		{
			// 进指定路径内搜索文件
#ifdef PATH__LZ_C100_IMG_FOLDER
			char t_path[] = PATH__LZ_C100_IMG_FOLDER;
			strcat(list_para->search_path, &t_path[2]);
#endif
...
static __s32 explorer_listview_create(H_WIN  list_win)
{
	...
	//GUI_WinSetFocusChild(list_win);		//frame win get focus.
	list_para->firmwareSize = GetFirmwareInfo_FileSize();

	if((list_para->media_type == RAT_MEDIA_TYPE_FIRMWARE) && list_para->firmwareSize)
	{
#if TIMEOUT_ID_TIMER_AutoUpdateFirmware
		list_para->fileTotal = list_para->rat.total;

		if(GUI_IsTimerInstalled(list_win, ID_TIMER_AutoUpdateFirmware))
		{
			GUI_ResetTimer(list_win, ID_TIMER_AutoUpdateFirmware, TIMEOUT_ID_TIMER_AutoUpdateFirmware, NULL);
		}
		else
		{
			GUI_SetTimer(list_win, ID_TIMER_AutoUpdateFirmware, TIMEOUT_ID_TIMER_AutoUpdateFirmware, NULL);
		}
...
void explorer_list_on_timer(__gui_msg_t *msg)
{
	...
	*/
	...
	if(ID_TIMER_AutoUpdateFirmware == msg->dwAddData1)
	{
		GUI_KillTimer(msg->h_deswin, ID_TIMER_AutoUpdateFirmware);

		if((Explorer_OpenFirmwareFile(list_para) == EPDK_FAIL) && (list_para->fileTotal > 1))
		{
			__gui_msg_t mymsg;
			mymsg.h_deswin = msg->h_deswin;
			mymsg.h_srcwin = NULL;
			mymsg.id = GUI_MSG_KEY;
			mymsg.dwAddData1 = GUI_MSG_KEY_RIGHT;
			mymsg.dwAddData2 = KEY_DOWN_ACTION;
			mymsg.dwReserved = 0;
			GUI_SendNotifyMessage(&mymsg);
			GUI_SetTimer(msg->h_deswin, ID_TIMER_AutoUpdateFirmware, TIMEOUT_ID_TIMER_AutoUpdateFirmware, NULL);
			list_para->fileTotal--;
		}
	}
