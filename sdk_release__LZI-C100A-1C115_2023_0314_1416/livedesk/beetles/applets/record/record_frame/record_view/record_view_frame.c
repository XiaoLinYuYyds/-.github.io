...
#define START_RECORD_IMMEDIATALLY 0
//#define TEST_CONTROL
	...
	if(KEY_UP_ACTION != msg->dwAddData2)
	{
		switch(msg->dwAddData1)
		{
			case GUI_MSG_KEY_ENTER:
				...
				{
					if(pviewFrm_ctl->pview->curMenuSt == FOCUSE_PRESS)
					{
			...
			case GUI_MSG_KEY_LEFT:
				...
				else if(pviewFrm_ctl->pview->curMenu == MENU_FORMAT)
				{
					...
					pviewFrm_ctl->pview->menuOsffset = 0;  //影响bottom下面的格式
					pviewFrm_ctl->pview->curMenuSt = FOCUSE_PRESS;
				}
...
static __s32 _rec_view_frm_manager_proc(__gui_msg_t *msg)
{
	...
	switch(msg->id)
	{
		...
		case RECORD_VIEW_FRAME_STARTRECORD:		//????????
		{
			prec_view_frame_t pviewFrm_ctl;
			pviewFrm_ctl = (prec_view_frame_t)GUI_WinGetAttr(msg->h_deswin);
		...
		case RECORD_VIEW_FRAME_STOPRECORD:		//?á??????
		{
			prec_view_frame_t pviewFrm_ctl;
			pviewFrm_ctl = (prec_view_frame_t)GUI_WinGetAttr(msg->h_deswin);
...
#if 1 == START_RECORD_IMMEDIATALLY
	pviewFrm_ctl->precordDoc->recState = RECORD_START;
#endif
	pviewFrm_ctl->precordDoc->recQuality = QUALITY_VERY_GOOD;
