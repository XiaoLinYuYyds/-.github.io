...
void _setting_general_get_res_id_mapping(__u32 *pMapping)
{
	...
	for(i = 0; i < SETTING_MENU_MAX; i++)
	{
		switch(i)
		{
			case SETTING_MENU_LANGUAGE ://语言
				pMapping[i] = m_eSettingResId_language;
				break;

			case SETTING_MENU_STYLE ://风格
				pMapping[i] = m_eSettingResId_style;
				break;

			case SETTING_MENU_INFO ://产品信息
			...
			case SETTING_MENU_AVOUT_FORMAT ://视频输出
				pMapping[i] = m_eSettingResId_tvoutSys;
				break;

			case SETTING_MENU_KEYTONE ://出版者
				pMapping[i] = m_eSettingResId_keytone;
				break;

			case SETTING_MENU_VOLUMESET ://音量
			...
			case SETTING_MENU_BL_TIME ://背光关闭时间
	...
	for(i = 0; i < SETTING_MENU_MAX; i++)
	{
		...
		switch(i)
		{
			...
			case SETTING_MENU_VOLUMESET:
				if(setting_reg_para)
				{
					__wrn("setting_reg_para = %d\n",setting_reg_para->volume);
					...
					{
						p_item_res->content_num = 10/*setting_reg_para->volume*/;
					}
				}
				else
				{
					__wrn("p_item_res->content_num = %d\n",p_item_res->content_num);//设置开机默认声音
					p_item_res->content_num = 10;
				}
...
static void setting_restore_default(__gui_msg_t *msg)
{
	...
	if(setting_reg_para)
	{
		...
		dsk_keytone_on();
		dsk_volume_set(10/*setting_reg_para->volume*/);	//音量恢复出厂设置
		{
			// 重置屏参
			setting_general_attr_t *general_attr;
			general_attr = (setting_general_attr_t *)GUI_WinGetAddData(msg->h_deswin);
