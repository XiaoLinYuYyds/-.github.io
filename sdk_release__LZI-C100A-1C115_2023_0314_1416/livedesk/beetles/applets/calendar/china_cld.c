...
/*月份数据表*/
static __u8  const day_code1[9] =
{
	0x0, 0x1f, 0x3b, 0x5a, 0x78, 0x97, 0xb5, 0xd4, 0xf3
};
static __u16 const day_code2[3] =
{
	0x111, 0x130, 0x14e
};

static __u8 const sky[][3] =
{
	"甲", "乙", "丙", "丁", "戊", "己", "庚", "辛", "壬", "癸",
};
static __u8 const earth[][3] =
{
	"子", "丑", "寅", "卯", "辰", "巳", "午", "未", "申", "酉", "戌", "亥",
};
static __u8 const monthcode[][3] =
{
	"一", "二", "三", "四", "五", "六", "七", "八", "九", "十", "冬", "腊",
};
static __u8 const nongliday[][3] =
{
	"初", "十", "廿", "三",
};
static char const ShengXiao[][3] =
{
...
static __u8 _get_chinacalendar(__u32  year, __u8 month, __u8 day, __u8 *p)
{
	...
	if(temp4 >= temp3)
	{
		...
		if(_get_moonday(month_p, table_addr) == 0)
		{
			temp1 = 29;  //小月29天
		}
		else
...
void Cld_GetChinaCalendarStr(__u32  year, __u8 month, __u8 day, char *str)
{
	...
	if(_get_chinacalendar(year, month, day, (__u8 *)NLyear) == 0)
	{
		return;
	}
	...
	if(NLyear[2] == 1)
	{
		eLIBs_memcpy(&str[6], (__u8 *)"正", 2);
	}
	else
	{
		eLIBs_memcpy(&str[6], (__u8 *)monthcode[NLyear[2] - 1], 2);
	}

	if(NLyear[3] > 10)
	{
		eLIBs_memcpy(&str[10], (__u8 *)nongliday[NLyear[3] / 10], 2);
	}
	else
	{
		eLIBs_memcpy(&str[10], (__u8 *)"初", 2);
	}
...
void Cld_GetChinaDayStr(__u32  year, __u8 month, __u8 day, char *str)
{
	...
	if(NLyear[3] > 10)
	{
		eLIBs_memcpy(&str[0], (__u8 *)nongliday[NLyear[3] / 10], 2);
	}
	else
	{
		eLIBs_memcpy(&str[0], (__u8 *)"初", 2);
	}
...
__u8 Cld_GetJieQiStr(__u32 year, __u8 month, __u8 day, char *str)
{
	...
	if(_get_jieqi(year, month, day, &JQdate) == 0)
	{
		return 0;
	}
...
#define STR_CHINACALENDAR_DEFAULT   "甲子(鼠)年正月初一"
	...
	if(NLyear[2] == 1)
	{
		eLIBs_memcpy(&str[10], (__u8 *)"正", 2);
	}
	else
	{
		eLIBs_memcpy(&str[10], (__u8 *)monthcode[NLyear[2] - 1], 2);
	}

	if(NLyear[3] > 10)
	{
		eLIBs_memcpy(&str[14], (__u8 *)nongliday[NLyear[3] / 10], 2);
	}
	else
	{
		eLIBs_memcpy(&str[14], (__u8 *)"初", 2);
	}
