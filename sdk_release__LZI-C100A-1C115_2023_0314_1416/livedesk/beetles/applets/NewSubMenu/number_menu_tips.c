	...
	switch(msg->id)
	{
		...
		case MSG_NUMBER_MENU_OP_ENTER:	//回车确认按键
		{
			...
			{	
				...
				if(key_up_number == 6)//为选中通用设置app时
				{
					key_number += 1;
				}
				if(key_number == 3)//按三次回车进入新的通用设置子窗口
				{
					...
					#endif
					#if 1//直接发送新添加的通用设置app的id创建mainwin窗口
				...
				{	
					__wrn("number_menu_tips.c key_number =%d  \n",key_number);
					number_menu_numbers_paint_all(msg,key_number);//刷新number次数显示内容
				}
			#endif
