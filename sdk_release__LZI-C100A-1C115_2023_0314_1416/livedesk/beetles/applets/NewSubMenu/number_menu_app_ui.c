...
*获取传入图片的数据 --> 传入图片ID索引
====================================================================================*/
 number_res_id_t *number_get_app_ui_para(SIZE rotate)
{
...
__s32 init_numbermenu_res(numbermenu_attr_t *number_menu_attr)
{
	...
	home_ui_para = home_get_ui_para(GUI_GetScnDir());//获取主窗口大小
	...
	for(i = 0; i < MAX_ITEM_NUM; i++)
	{
		...
		{
			number_menu_attr->number_unfocus_bmp[i] = dsk_theme_open(main_ui_para[i].app_unfocus_bmp_id);	//不选择的图片索引
		}

		if(NULL == number_menu_attr->number_focus_bmp[i])
		{
			number_menu_attr->number_focus_bmp[i] = dsk_theme_open(main_ui_para[i].app_focus_bmp_id);		//选择的图片索引
		}
	}
...
void paint_numbermain_one_app(numbermenu_attr_t *number_menu_attr, __s32 index, __s32 bfocus)
{
	...
	if(!(app_paint_flag & NUMBER_MASK_PaintFlag_MEMDEV_Nonuse))	//
	{
		...
		app_draw_mem = GUI_MEMDEV_Create(0, 0, screen_width, screen_height); //伪内存创建，大小,使图片绘制更快
	...
	#if 1	//绘制矩形
	{
		...
		GUI_BMP_Draw(app_pbmp, app_bmp_x, app_bmp_y);	//传入app_pbmp图片数据进行绘制
		{
			...
			dsk_langres_get_menu_text(number_ui_para[index].app_lang_id, app_text, sizeof(app_text));	//文本显示函数
