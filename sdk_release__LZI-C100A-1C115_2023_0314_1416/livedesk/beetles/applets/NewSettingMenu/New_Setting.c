...
static void New_Setting_init(New_Setting_ctrl_t *New_Setting_ctrl)
{
	...
	#endif
	#if 1	//添加进度条显示
		if(!New_Setting_ctrl->New_Setting_progressbar_bmp)
		{
			New_Setting_ctrl->New_Setting_progressbar_bmp = theme_open(ID_MOVIE_VOLUME_PROG_BG_UF_BMP);//进度条矩形框背景图
		}
	#endif
...
static void New_Setting_uninit(New_Setting_ctrl_t *New_Setting_ctrl)
{
...

	{
		...
		if(New_Setting_ctrl->New_Setting_progressbar_bmp)
		{
			dsk_theme_close(New_Setting_ctrl->New_Setting_progressbar_bmp);//进度条矩形框背景图
			New_Setting_ctrl->New_Setting_progressbar_bmp = NULL;
		}
		#endif
 ...
 __s32 New_Setting_paint_one_item(__gui_msg_t *msg,__u32 num)
{
	...
	void  	*progress_theme;	//存放获取的进度条图片使用资源数据
	...
	__wrn("New_Setting_ctrl layer = %d\n",New_Setting_ctrl->New_Settings_para->layer);
	...
	//while(sys_time--)
	{
		...
		#endif
		{
			progress_theme = theme_hdl2buf(New_Setting_ctrl->New_Setting_progressbar_bmp);			//获取图片使用资源数据
			GUI_BMP_Draw(progress_theme, 0, 30);//绘制文本控件number:00显示函数矩形背景图和显示坐标，防止number的字符串被覆盖问题
			//GUI_BMP_Draw(number_theme, 0, 0);//绘制文本控件number:00显示函数，防止number的字符串被覆盖问题
...
static __s32 _New_Setting_proc(__gui_msg_t *msg)
{
	...
	switch(msg->id)
	{
		...
		case GUI_MSG_DESTROY: //销毁窗口
		{
				...
				New_Setting_para = (new_setting_para_t *)GUI_WinGetAttr(msg->h_deswin);		//获取窗口的属性
				...
				__wrn("New_Setting_ctrl->New_Settings_para free size = %d\n",New_Setting_ctrl->New_Settings_para);
				__wrn("New_Setting_ctrl free size = %d\n",New_Setting_ctrl);
				__wrn("New_Setting_para  free size = %d\n",New_Setting_para);//打印结构体是否释放内存
...
H_WIN New_Setting_app_win_create(H_WIN h_parent, new_setting_para_t *para)
{
	...
	#endif
	GUI_LyrWinGetFB(para->layer, &fb);				//获取图层窗口大小然后赋值给FB
