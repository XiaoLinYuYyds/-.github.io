...
H_LYRIC new_music_lyrics(music_lyrics_para_t *para)
{
	...
	//GUI_ClearRect(hLyric->UI.area.x,hLyric->UI.area.y,hLyric->UI.area.x + hLyric->UI.area.width, hLyric->UI.area.y + hLyric->UI.area.height);
	{
		...
		X =  hLyric->UI.area.x;
		Y =  hLyric->UI.area.y;
		__msg("hLyric->UI.area.height = %d\n", hLyric->UI.area.height);
		__msg("hLyric->UI.area.width  = %d\n", hLyric->UI.area.width);
		__msg(" font_height  = %d\n", 		   font_height);
		__msg("hLyric->line_count = %d\n",     hLyric->line_count);
		...
		for(i = 0; i < hLyric->line_count; i++)
		{
			...
			Y += font_height;
			__msg(" hLyric->line[i].area.x0  = %d\n", hLyric->line[i].area.x0);
			__msg(" hLyric->line[i].area.y0  = %d\n", hLyric->line[i].area.y0);
			__msg(" hLyric->line[i].area.x1  = %d\n", hLyric->line[i].area.x1);
			__msg(" hLyric->line[i].area.y1  = %d\n", hLyric->line[i].area.y1);
			__msg(" Y   = %d\n", Y);
		}
	}
	return hLyric;
