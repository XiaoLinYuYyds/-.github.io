...
//是否显示歌曲文件信息，如艺术家，文件大小等
#define PLAYER_FILE_INFO			　　
...
//原唱、音乐模式
__s32 DrawMusicAcompStatus(__s32 channel)
{
	char text[128];
	music_headbar_ui_t *headbar_ui;
	GUI_RECT gui_rect;
	char vol_text[3];
	__wrn("DrawMusicAcompStatus...\n");
	...
	gui_rect.x0 = headbar_ui->acomp_rect.x;
	gui_rect.y0 = headbar_ui->acomp_rect.y;
	gui_rect.x1 = headbar_ui->acomp_rect.x + headbar_ui->acomp_rect.width - 1;
	gui_rect.y1 = headbar_ui->acomp_rect.y + headbar_ui->acomp_rect.height - 1;
...
__s32 DrawMsicButtonHintText(__s32 TextID)
{
	...
	GUI_UC_SetEncodeUTF8();
	GUI_DispStringInRect(TextBuf, &text_rect, GUI_TA_LEFT | GUI_TA_VCENTER);
	return EPDK_OK;
}
