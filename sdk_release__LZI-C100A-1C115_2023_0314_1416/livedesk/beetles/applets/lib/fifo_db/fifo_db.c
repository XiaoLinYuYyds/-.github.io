...
__s32  get_item_content_from_fifo_db(const char *title, const char *key, void *buf)
{
	...
	for(i = 0 ; i < head.content_num ; i++)
	{
		...
		if(0 == g_strncmp(key, item_info.key , FIFO_DB_KEY_MAX_LEN))
		{
			...
			__wrn("item found,key=%s, head.content_num=%d\n", key, head.content_num);
			ret = 0;
			goto exit;
		}
	}

	__wrn("item not found to delete,key=%s, head.content_num=%d\n", key, head.content_num);
	ret = -1;
	goto exit;
exit:
...
__s32  add_item_to_fifo_db(const char *title, const char *key, const void *buf)
{
	...
	if(1 == item_exist)
	{
		__wrn("item alreadly exist, key=%s, head.content_num=%d\n", key, head.content_num);
		ret = 0;
		goto exit;
	}
	...
	if(head.content_num >= head.max_content_num)
	{
		...
		head.content_num--;
		__wrn("delete last item, head.content_num=%d\n", head.content_num);
	}
	...
	__wrn("item added, key=%s, head.content_num=%d\n", key, head.content_num);
	ret = 0;
	goto exit;
exit:
	...
	for(i = 0 ; i < head.content_num ; i++)
	{
		...
		if(0 == g_strncmp(key, item_info.key , FIFO_DB_KEY_MAX_LEN))
		{
			...
			for(; i < head.max_content_num; i++)
			{
				g_fseek(fp, sizeof(head) + i * sizeof(item_info_t), SEEK_SET);
				g_fwrite(&item_info, 1, sizeof(item_info_t), fp);
				__wrn("write i to empty item, i=%d\n", i);
			}
			...
			__wrn("item deleted,key=%s, head.content_num=%d\n", key, head.content_num);
			ret = 0;
			goto exit;
		}
	}
...
static __s32  create_fifo_db_i(const char *title, __u32 item_max_nr, __u32 item_size)
{
	...
	if(fp == NULL)
	{
		__wrn("Fail in creating fifo db file, file=%s\n", file);
		goto error;
	}
