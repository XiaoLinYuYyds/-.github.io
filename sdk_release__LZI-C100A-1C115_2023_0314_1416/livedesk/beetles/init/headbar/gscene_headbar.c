...
static __s32 cb_shbar_mwin(__gui_msg_t *msg)
{
	switch(msg->id)
	{
		case GUI_MSG_CREATE:
		{
			...
			if(!FONT16)
			{
				FONT16 = GUI_SFT_CreateFont(BEETLES_FONT_SIZE_INT, BEETLES_APP_ROOT"res\\fonts\\font"BEETLES_FONT_SIZE_STR".sft");
			}
...
__s32 gscene_hbar_set_bt(__bool staOn)
{
	...
	__msg_jjl("gscene_hbar_set_bt(%d)\n", staOn);
	eLIBs_memset(&msgex, 0, sizeof(__gui_msg_t));
	msgex.id 			= HBAR_BT_PAINT;
	msgex.h_srcwin 		= NULL;
	msgex.h_deswin		= headbar_win;
	msgex.dwAddData1	= staOn;
	msgex.dwAddData2	= 0;
	GUI_SendMessage(&msgex);
	return 0;
}
