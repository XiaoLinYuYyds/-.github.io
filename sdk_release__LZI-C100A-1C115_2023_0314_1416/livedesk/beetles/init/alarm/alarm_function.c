...
static __u32 _alarm_get_time_minute(__time_t time)
{
...
static __s32 alarm_get_near_on_time(__time_t *time, __s16 *alarmIndex)
{
...
static __s32 alarm_get_near_on_time_standby(__time_t *time, __date_t *date)
{
	reg_system_para_t 	*res_sys = NULL;
	__time_t			curtime;
	__u16				week, mDay;
...
static __s32 standby_alarm_start(__time_t alarmTime, __date_t date)
{
	if(alarm_hdle != NULL)
	{
		alarm_close();
	...
	__wrn("standby alarm:%d:%d, day:%d\n", alarmTime.hour, alarmTime.minute, date.day);
	return EPDK_OK;
}
...
__bool alarm_check_run(void)
{
	...
	{
		__wrn("%02d:%02d, alarm to app!\n", res_sys->alarm[alarmIndex].alarm_time.hour, res_sys->alarm[alarmIndex].alarm_time.minute);
