...
#define POINT_BMP_WIDTH			21
#define POINT_BMP_HEIGHT			19
	...
	switch(msg->id)
	{
		...
		case GUI_MSG_PAINT:
		{
			...
			if(GUI_LYRWIN_STA_SUSPEND != GUI_LyrWinGetSta(layer))
			{
				...
				rcrect.x0 = 0;
				rcrect.y0 = 0;
				rcrect.x1 = fbrect.width - 1;
				rcrect.y1 = fbrect.height - 1;
				GUI_SetBkColor(0);
				...
				GUI_DrawLine(rcrect.x0, rcrect.y0 + 8, rcrect.x1, rcrect.y0 + 8);
				GUI_DrawLine(rcrect.x0, rcrect.y1 - 8, rcrect.x1, rcrect.y1 - 8);
				GUI_CharSetToEncode(EPDK_CHARSET_ENM_UTF8);
				rcrect.y1 = rcrect.y0 + fbrect.height * 2 / 3 - 1;
		...
		case GUI_MSG_TIMER:
		{
			...
			if(GUI_LYRWIN_STA_SUSPEND != GUI_LyrWinGetSta(layer))
			{
				GUI_LyrWinSel(layer);
				GUI_WinGetClientFBRect(msg->h_deswin, &fbrect);
				rcrect.x0 = (fbrect.width - POINT_BMP_WIDTH * 5) / 2;
				rcrect.y0 = fbrect.height * 2 / 3;
				rcrect.x1 = rcrect.x0 + POINT_BMP_WIDTH * 5 - 1;
				rcrect.y1 = rcrect.y0 + POINT_BMP_HEIGHT - 1;
