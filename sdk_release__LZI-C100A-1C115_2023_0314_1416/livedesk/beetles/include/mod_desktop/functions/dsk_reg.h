...
#define CONST_DispSaturability_Default		50
	...
	typedef struct
	{
		...
		__u32   last_play_app;          //最后一次运行的ap类型，0:音乐,1:视频, 2:电子书 3:图片
		__u32   last_app_play_storage[m_eMediaTypeList_max];  //各个ap最后一次播放的媒体介质，0:tf卡 1:u盘
...
#define SIZE_sTunerAtvList_shortcutIdx	10
	typedef struct
	{
		S_TUNER_ATV		m_sTunerAtvList[SIZE_sTunerAtvList];	// 存储搜索到的频道列表
