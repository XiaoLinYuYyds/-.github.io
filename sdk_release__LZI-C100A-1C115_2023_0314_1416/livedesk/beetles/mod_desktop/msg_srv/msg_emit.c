...
static __s32 key_msg_cb(void *msg)
{
	...
	for(i = 0; i < pPacket->event_cnt; i++)
	{
		...
		if(pEventFrame->type == EV_KEY)
		{
			...
			if(pEventFrame->code & MASK_IR_KPAD)
			{
				pEventFrame->code &= ~MASK_IR_KPAD;
				t_guiMsgKeyType = MASK_GUI_MSG_KEY_TYPE_IR;
			}

			__msg("pEventFrame->code=0x%x\n", pEventFrame->code);
		...
		/*		case KPAD_FAVORFILE:
				{
					__wrn("KPAD_FAVORFILE\n");
					pmsg->type	= GUI_MSG_KEY;
					pmsg->id	= GUI_MSG_KEY_FAVORFILE;
					break;
				}*/
		/*		case KPAD_FAVORPLAY:
				{
					__wrn("KPAD_FAVORPLAY\n");
					pmsg->type	= GUI_MSG_KEY;
					pmsg->id	= GUI_MSG_KEY_FAVORPLAY;
					break;
				}
*/
				case KPAD_NUM0:
				...
				case KPAD_SUBMENU:
				{
					__wrn("KPAD_SUBMENU\n");
					pmsg->type 	= GUI_MSG_KEY;
					pmsg->id 	= GUI_MSG_KEY_SUBMENU;
					break;
				}

				case KPAD_PLAY_PAUSE:
				{
					__wrn("KPAD_PLAY_PAUSE\n");
					pmsg->type 	= GUI_MSG_KEY;
				...
				case KPAD_LOCK:
				{
					__wrn("KPAD_LOCK\n");
					pmsg->type 	= GUI_MSG_KEY;
					pmsg->id 	= GUI_MSG_KEY_LOCKSCREEN;
					break;
				}
				...
				case KPAD_FB_MULTI:
				{
					__wrn("KPAD_NUMLOCK\n");
					pmsg->type 	= GUI_MSG_KEY;
					pmsg->id 	= GUI_MSG_KEY_FB_MULTI;
					break;
				}

				case KPAD_SEARCHFREQ:
				{
					__wrn("KPAD_ECHOUP\n");
					pmsg->type 	= GUI_MSG_KEY;
					pmsg->id 	= GUI_MSG_KEY_SEARCHFREQ;
					break;
				}
				case KPAD_SUBMODE:
				{
			...
			/*	case KPAD_LED:
				{
					__wrn("KPAD_LED\n");
					pmsg->type	= GUI_MSG_KEY;
					pmsg->id	= GUI_MSG_KEY_LED;
					break;
				}	
*/
			// ************************************************** 
				case KPAD_SHIFT_POWER:
				{
					__wrn("KPAD_NUMLOCK\n");
					pmsg->type	= GUI_MSG_KEY;
					pmsg->id	= GUI_MSG_KEY_SHIFT_POWER;
					break;
				}
				case KPAD_SHIFT_SELTIME:
				{
					__wrn("KPAD_NUMLOCK\n");
					pmsg->type	= GUI_MSG_KEY;
					pmsg->id	= GUI_MSG_KEY_SHIFT_SELTIME;
					break;
				}
				case KPAD_SHIFT_SUBTITLE:
				{
					__wrn("KPAD_NUMLOCK\n");
					pmsg->type	= GUI_MSG_KEY;
					pmsg->id	= GUI_MSG_KEY_SHIFT_SUBTITLE;
					break;
				}
				case KPAD_SHIFT_ANGLE:
				{
					__wrn("KPAD_NUMLOCK\n");
					pmsg->type	= GUI_MSG_KEY;
					pmsg->id	= GUI_MSG_KEY_SHIFT_ANGLE;
					break;
				}
				case KPAD_SHIFT_CAPTION:
				{
					__wrn("KPAD_NUMLOCK\n");
					pmsg->type	= GUI_MSG_KEY;
					pmsg->id	= GUI_MSG_KEY_SHIFT_CAPTION;
					break;
				}
				case KPAD_SHIFT_STOP:
				{
					__wrn("KPAD_NUMLOCK\n");
					pmsg->type	= GUI_MSG_KEY;
					pmsg->id	= GUI_MSG_KEY_SHIFT_STOP;
					break;
				}
				case KPAD_SHIFT_ADD10:
				{
					__wrn("KPAD_NUMLOCK\n");
					pmsg->type	= GUI_MSG_KEY;
					pmsg->id	= GUI_MSG_KEY_SHIFT_ADD10;
					break;
				}
				case KPAD_SHIFT_SF:
				{
					__wrn("KPAD_NUMLOCK\n");
					pmsg->type	= GUI_MSG_KEY;
					pmsg->id	= GUI_MSG_KEY_SHIFT_SF;
					break;
				}
				case KPAD_SHIFT_PROGRAM:
				{
					__wrn("KPAD_NUMLOCK\n");
					pmsg->type	= GUI_MSG_KEY;
					pmsg->id	= GUI_MSG_KEY_SHIFT_PROGRAM;
					break;
				}
				case KPAD_SHIFT_SYS:
				{
				...
				case KPAD_SHIFT_AB:
				{
					__wrn("KPAD_NUMLOCK\n");
					pmsg->type	= GUI_MSG_KEY;
					pmsg->id	= GUI_MSG_KEY_SHIFT_AB;
					break;
				}
				case KPAD_SHIFT_USB_CARD:
				{
					__wrn("KPAD_NUMLOCK\n");
					pmsg->type	= GUI_MSG_KEY;
					pmsg->id	= GUI_MSG_KEY_SHIFT_USB_CARD;
					break;
				}
			...
			pmsg->id |= t_guiMsgKeyType;
			__msg("GUI_MSG_KEY_ val=0x%X, pmsg->type=%d\n", pmsg->id, pmsg->type);
			__msg("pmsg->reserved=%d\n", pmsg->reserved);
			esKRNL_QPost(emit_ctr.psys_msg_queue, pmsg);
		}
	}
	...
	while(1)
	{
		...
		else	/* system 消息 */
		{
			...
			else if((usrmsg & 0x0000ffff) == KMSG_USR_SYSTEM_FS_PLUGOUT)	//FS表示文件系统，目前用于TF卡插拔操作
			{
				...
				__wrn("DSK_MSG_FS_PART_PLUGOUT, pmsg->data=0x%x\n", pmsg->data);
			}
			else
			{
				__msg("*************usrmsg = %d***************\n", usrmsg);
