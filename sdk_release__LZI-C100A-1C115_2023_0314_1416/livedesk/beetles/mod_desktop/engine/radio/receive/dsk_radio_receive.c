		...
		if(dsk_radio_rcv->search_flag == 0x01)//搜索中
		{
			...
			else  //result == cur_freq
			{
				...
				else  //dsk_radio_rcv->auto_maual_mode == DSK_RADIO_SEARCH_AUTO
				{
					...
					dsk_radio_rcv_autosearch_suspend();	// 搜索到一个频道后,播放2.5秒再搜索下一个频道.
#endif
				}
			}
		}
...
#if SP_FM_WhenAutoSearch_PlayFreq
	                     & (~MASK_DskRadioAutoSearch_Suspend)
#endif
	                     ;
	return 0;
}
...
__s32 dsk_radio_rcv_search_matching_frep(__u32 freq)
{
	__u32 i = 0;
	__s32 channelid = -1;
	...
	for(i = 0; i < dsk_radio_rcv_get_search_result_vn(); i++)
	{
		__wrn("dsk_radio_rcv = %d", dsk_radio_rcv->search_result.freq_list[i].freq);

		if(dsk_radio_rcv->search_result.freq_list[i].freq == freq)
		{
			channelid = i + 1;
			return channelid;
		}
...
void dsk_radio_rcv_autosearch_suspend(void)
{
	dsk_radio_rcv->search_flag |= MASK_DskRadioAutoSearch_Suspend;
}

void dsk_radio_rcv_autosearch_resume(void)
{
	dsk_radio_rcv->search_flag &= ~MASK_DskRadioAutoSearch_Suspend;
}
