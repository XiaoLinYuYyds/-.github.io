...
extern __u32 power_count;
extern __s32 g_getEchoLevel;
...
__s32 dev_matrix_key_ioctrl(__hdle hKey, __u32 cmd, __s32 aux, void *pbuffer)
{
	...
	switch(cmd)
	{
		...
		case DRV_KEY_CMD_SET_POWER_FLAGS:
		{
			__here__;
			power_count = 1;
			break;
		...
		case DRV_KEY_CMD_GET_DEBUG_INFO :
		{
#if SP_GetDebugInfo_KeyAdSampling

			switch(aux)
			{
				case OP_GetDebugInfo_X1 :
					*((PS_ADSampling)pbuffer) = g_sAdSampling_x1;
					ret = EPDK_OK;
					break;

				case OP_GetDebugInfo_X2 :
					*((PS_ADSampling)pbuffer) = g_sAdSampling_x2;
					ret = EPDK_OK;
					break;
		...
		case DRV_KEY_CMD_DET_MIC_PLUG_EN:
		{
			if(m_eGetStaIdx_mic < m_eGetStaIdx_total)
			{
				g_sGetSta[m_eGetStaIdx_mic].unlock = FLAG__PlugStaChangeDetEn;
				ret = EPDK_OK;
			}
			break;
		}

		case DRV_KEY_CMD_DET_MIC_PLUG_DIS:
		{
			if(m_eGetStaIdx_mic < m_eGetStaIdx_total)
			{
				g_sGetSta[m_eGetStaIdx_mic].unlock = ~FLAG__PlugStaChangeDetEn;
		...
		case DRV_KEY_CMD_DET_AUX_PLUG_EN:
		{
			if(m_eGetStaIdx_aux < m_eGetStaIdx_total)
			{
				g_sGetSta[m_eGetStaIdx_aux].unlock = FLAG__PlugStaChangeDetEn;
				ret = EPDK_OK;
			}
			break;
		}

		case DRV_KEY_CMD_DET_AUX_PLUG_DIS:
		{
			if(m_eGetStaIdx_aux < m_eGetStaIdx_total)
			{
				g_sGetSta[m_eGetStaIdx_aux].unlock = ~FLAG__PlugStaChangeDetEn;
