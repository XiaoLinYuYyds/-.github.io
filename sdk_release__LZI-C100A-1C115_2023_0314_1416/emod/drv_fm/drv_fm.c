...
__s32 DRV_FM_MIoctrl(__mp *mp, __u32 cmd, __s32 aux, void *pbuffer)
{
	switch(cmd)
	{
		case DRV_CMD_PLUGIN:
		{
...
#if (!SP_SimIIC)
			fm_drv.fm_dev.iic_file = eLIBs_fopen("b:\\BUS\\TWI2", "r+");
			...
			//Tarring
			eLIBs_fioctrl(fm_drv.fm_dev.iic_file, TWI_SET_SCL_CLOCK, 0, (void *)TWI_SCLK_100KHZ);
#endif
			...
			fm_drv.used = 1;
			fm_drv.fm_type = __FM_TYPE_TOTAL;
