...
static void LCD_cfg_panel_info(__panel_para_t *info)
{
	...
	//屏的HV模块相关信息
	info->lcd_hv_if          = 1;//lcd_hv_if = 1,rgb888, 分三次传送, 0=24bit
	info->lcd_hv_hspw        = 5;//20;//1--同步信号宽度
	...
	//屏的HV配置信息
	info->lcd_hbp            = 10;//3;//3;					//看手册配置
	info->lcd_ht             = 1415;//info->lcd_hbp + 1 + (info->lcd_x + 1)*3+2 + 10;		//总cycle数//info->lcd_hbp + 1 + (info->lcd_x + 1)*3+2 + 100;		//总cycle数
	info->lcd_vbp            = 10;//2;//2;
...
static __s32 LCD_open_flow(__u32 sel)
{
	...
	//LCD_OPEN_FUNC(sel, LCD_open_cmd	,650); 	//通过IO配置LCD模组为工作模式，并延时100ms
	LCD_OPEN_FUNC(sel, LCD_open_cmd	, 1000); 	//通过IO配置LCD模组为工作模式，并延时100ms
