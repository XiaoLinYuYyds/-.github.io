...
__drv_power_fuelguage_t fuelguage[m_eDrvPowerFuelguageIdx_total];

__hdle hPowerOn = NULL;
__hdle hChargeStatus = NULL;

extern __s32 g_adc_exp;
extern __s32 g_adc_exp_tp;
...
__s32 DRV_PowerMInit(void)
{
	...
	eLIBs_memset(fuelguage, 0, sizeof(fuelguage));
	...
	{
		//获取AD采样后的系数，对生产批次不同，系数也可能不同
		if(EPDK_OK == esCFG_GetKeyValue("power_para", "adc_exp_tp", &g_adc_exp_tp, 1))
		{
			__msg("***********g_adc_exp_tp********=%d\n", g_adc_exp_tp);
		}
		else
		{
			__msg("get key val fail...\n");
	...
	// 初始化REG_CTRL
	RK_Init();
	RK_SetFifoDepth(RK_FIFO_DEPTH_01);
	RK_SetSampleRate(RK_SAMPLE_RATE_01K);
	RK_EnableInt(RK_DATA_IRQ);
	RK_SelectChannel(RK_CHANNEL_Y1);
	RK_ClearFifo();
	eLIBs_printf("***********************WRN use TP KEY************************\n");
#endif
	return EPDK_OK;
err_out:
