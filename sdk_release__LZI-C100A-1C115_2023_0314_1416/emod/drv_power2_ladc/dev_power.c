...
static __s32 dsk_send_rmmod_usbh_cmd(void)
{
...
static __s32 __power_decr_freq(void)
{
	...
	//}
	__here__;
	//esCLK_SetSrcFreq(CSP_CCM_SYS_CLK_CORE_PLL, 2*6*1000*1000);
	//esCLK_SetSrcFreq(CSP_CCM_SYS_CLK_SDRAM_PLL, 2*1*1000*1000);
	//esPWM_LockCpuFreq();
...
__s32 DEV_PowerIoctrl(__hdle hPower, __u32 cmd, __s32 aux, void *pbuffer)
{
	...
	switch(cmd)
	{
		...
		case DRV_POWER_CMD_POWER_OFF:
		{
			...
			powerdrv.hTimer = NULL;
			//__msg("close lcd begin\n");
			//__close_lcd();
			{
				ES_FILE 			 *p_key = NULL;
				/* open display driver */
			...
			*tmpAddr = tmpData;
			...
			//cpu_sr = cpu_sr;
			//死循环,挂死CPU
			__msg("power no use, cpu dumping\n");
			__here__;
			DEV_PowerOnProcess();
			__msg("DEV_PowerOnProcess\n");
			__here__;
		...
		case DRV_POWER_CMD_GET_FUELGUAGE:
		{
			...
			eLIBs_memcpy(pbat, &fuelguage[m_eDrvPowerFuelguageIdx_slow], sizeof(__drv_power_fuelguage_t));
			ret = EPDK_OK;
			break;
		}

		case DRV_POWER_CMD_GET_FUELGUAGE_FAST:
		{
			__drv_power_fuelguage_t *pbat = (__drv_power_fuelguage_t *)pbuffer;
			eLIBs_memcpy(pbat, &fuelguage[m_eDrvPowerFuelguageIdx_fast], sizeof(__drv_power_fuelguage_t));
			ret = EPDK_OK;
			break;
		}
