...
static	__s32 New_explorer_ListView_creat(H_WIN list_win)
{
			...
			__wrn("\nlist_item_number = %d\n",list_item_number);	
				...
				{//文件名显示矩形坐标TextRect（59，0，338，30）
					...
					/*else
					{
						...
						List_TextRect.y0 =	list_item_number*50 + 8 + TextRect.y;	//在列表项目条目显示坐标 间隔为50，项目条数量*50，3为列表显示区域坐标y轴起始坐标
						List_TextRect.x1 =	156 + (TextRect.x + TextRect.width);			//文本名显示的矩形宽为固定338 + 59
	...
	#endif
	#if 1//获取媒体类型文件的文件名，然后拷贝到结构体数组list_para->FileName中
