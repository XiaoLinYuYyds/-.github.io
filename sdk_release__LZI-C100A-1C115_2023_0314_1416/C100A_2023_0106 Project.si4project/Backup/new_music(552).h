...
typedef	struct app_new_music_create_para
{
	...
	__u8 	auto_play_timmer_id;//播放器定时器id
	__u8 	key_timmer_id;		//按键检测定时器id
	__u8 	ff_rr_timmer_id;	//快进或后退定时器id
	
	rat_media_type_t 	 media_type;	//存放获取到磁盘的文件类型
	root_para_t  		*root_para;		//存放root根目录结构体变量
	__u32	curplay_index;	//当前播放的音乐文件索引id
	__u32	lastplay_index;	//上一次播放的音乐文件索引id
...
typedef	struct new_music_ctrl_para
{
	...
	new_music_scene_t	scene_uipara;//测试频谱图
