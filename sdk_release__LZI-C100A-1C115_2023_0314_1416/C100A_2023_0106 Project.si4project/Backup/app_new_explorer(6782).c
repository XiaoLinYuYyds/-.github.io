...
static __s32 new_explorer_scene_create(__gui_msg_t *msg)
{
	...
	new_explorer_ListPara.view_mode		= new_explorer_ctrl->view_mode;		//使用liest列表显示模式还是正方形显示模式
	...
	new_explorer_ctrl->new_list_layer = new_explorer_32bpp_layer_create();		//列表框使用32位bpp图层创建
	__wrn("----app new explorer manager window new_list_layer creat begin is 2----\n");
	new_explorer_ctrl->h_new_explore  = new_explorer_list_framewin_creat(msg->h_deswin,&new_explorer_ctrl);//创建framewin子窗口
	...
	GUI_LyrWinSetSta(new_explorer_ctrl->new_list_layer, GUI_LYRWIN_STA_ON);//图层开启
	GUI_LyrWinSetTop(new_explorer_ctrl->new_list_layer);				   //图层停止
	return EPDK_OK;
}
