...
static __s32 new_music_paint_one_item(__gui_msg_t *msg)
{
	...
	#if 1 //绘制当前时间
	{
		GUI_RECT cur_time_rect;
		char buf[]="00:04:06";
		GUI_SetFont(new_music_ctrl->framewin_create_para->font);//设置文本
		...
		GUI_DispStringInRect("00:04:05", &cur_time_rect, GUI_TA_VCENTER | GUI_TA_HCENTER);
	...
	if(KEY_UP_ACTION == msg->dwAddData2){//按键松开动作获取，第一次进入的按键执行功能
		...
		switch(msg->dwAddData1){
			...
			case GUI_MSG_KEY_LEFT:{/*上一首*/
				...
				this->framewin_create_para->curplay_index = robin_npl_get_cur();//获取当前播放的文件索引id
				app_new_music_cmd2para(msg->h_deswin, MSG_PLAY_OP_UP, this->framewin_create_para->curplay_index, 0);//向父窗口发送命令上一首命令
			}
			break;
