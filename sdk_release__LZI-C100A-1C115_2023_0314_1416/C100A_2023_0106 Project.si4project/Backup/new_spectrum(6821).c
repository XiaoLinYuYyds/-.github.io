...
#ifndef  __new_spectrum_c
#define  __new_spectrum_c


#include "beetles_app.h"

#include "new_spectrum_i.h"
...
//频谱控制寄存器
__bool NEW_RegisterSpectrumControl(void)
{
	__gui_winclass_t  WndClass;//窗口类的结构体
	__wrn("NEW_RegisterSpectrumControl...\n");
	WndClass.spClassName = CTRL_SPECTRUM;		//类名
	WndClass.dwStyle     = WS_NONE;				//类的窗口风格
	WndClass.dwExStyle   = WS_EX_NONE;			//扩展风格
	WndClass.WinProc     = _spectrumCtrlProc;	//窗口处理函数
	return GUI_AddNewControlClass(&WndClass) == EPDK_OK;
}


//删除频谱控制寄存器
__bool NEW_UnRegisterSpectrumControl(void)
{
	return(GUI_DeleteControlClass(CTRL_SPECTRUM));
}

//频谱设置数值
__s32  NEW_SPECTRUM_SetValue(H_WIN hwin, __s32 *value_array)
{
	__u32  i;
	NEW_HTRUM  htrum = (NEW_HTRUM) GUI_CtrlWinGetAddData(hwin);
	__msg("\n****NEW_SPECTRUM_SetValue****\n");
	if(!_is_valid_handle(htrum))
	{
		__wrn("\n****_is_valid_handle is false****\n");
		return -1;
	}

	if(!spectrum_is_waking_i(htrum))
	{
		__wrn("\n****spectrum_is_waking_i is false****\n");
		return -1;
	}
...
{
	__u32	  dwStyle;
	NEW_HTRUM_t  *htrum = NULL;
	H_WIN     hwin;
	hwin = msg->h_deswin;
	dwStyle = GUI_WinGetStyle(hwin);
	htrum = (NEW_HTRUM)GUI_CtrlWinGetAddData(hwin);
	...
	switch(msg->id)
	{
		...
		{
			NEW_HTRUM_t  *htrum = NULL;
			htrum = (NEW_HTRUM_t *) g_malloc(sizeof(NEW_HTRUM_t));
			__wrn("_spectrumCtrlProc GUI_MSG_CREATE\n");
			if(htrum == NULL)
			{
				return EPDK_FAIL;
			}

			g_memset(htrum, 0, sizeof(NEW_HTRUM_t));

			if(_init_htrum(htrum, hwin))//初始化该结构体
			{
				_release_res(htrum);//释放结构体所有数据
				g_free(htrum);		//释放结构体内存空间
				htrum = NULL;
				return EPDK_FAIL;
			}

			GUI_CtrlWinSetAddData(hwin, (__u32)htrum);//控制窗口设置添加一个数据

			if(dwStyle & WS_VISIBLE)
			{
				/* draw spectrum */
				htrum->is_showing = EPDK_TRUE;
				_my_GUI_InvalidateRect(hwin, NULL, EPDK_TRUE);//指定区域刷新
			}
		...
		case GUI_MSG_DESTROY:
			__wrn("_spectrumCtrlProc GUI_MSG_DESTROY\n");
			if(htrum != NULL)
			{
				_release_res(htrum);
				g_free(htrum);
		...
		case GUI_MSG_PAINT://绘制频谱ui
			__wrn("_spectrumCtrlProc GUI_MSG_PAINT\n");
			_ondraw(htrum, hwin);
			return EPDK_OK;

		case GUI_MSG_WIN_SLEEP:
			if(!spectrum_is_waking_i(htrum))                  // has slept休眠
			{
				return EPDK_OK;
			}
		...
		case GUI_MSG_WIN_WAKEUP:
			if(spectrum_is_waking_i(htrum))                   // has waken up唤醒
			{
				return EPDK_OK;
			}
...
//检测句柄是否有效
static __bool _is_valid_handle(NEW_HTRUM htrum)
{
	if(htrum != NULL)
	{
		return EPDK_TRUE;
...
{
	new_spectrum_para_t *para_p;
	RECT        rect;
	para_p = (new_spectrum_para_t *)GUI_WinGetAttr(hwin);
...
//绘制频谱所有ui
static __s32  _ondraw(NEW_HTRUM htrum, H_WIN hwin)
{
	__wrn("_ondraw...\n");
	if(htrum->is_internal_cmd)                                  // internal cmd to draw
	{
		...
		{
			__msg("NOT support wake up in intertal cmd 1 ...\n");
			_save_locale(htrum);
			htrum->is_internal_cmd = EPDK_FALSE;
			return 0;
		}

		if(!htrum->is_waking && htrum->old_is_waking)           // to sleep休眠
		{
			__msg("NOT support wake up in intertal cmd 2 ...\n");
			_save_locale(htrum);
			htrum->is_internal_cmd = EPDK_FALSE;
			return 0;
		}

		if(htrum->is_waking && !htrum->old_is_waking)           // to wake up唤醒
		{
			__msg("NOT support wake up in intertal cmd.\n");
			_save_locale(htrum);
			htrum->is_internal_cmd = EPDK_FALSE;
			return 0;
		}

		if(htrum->is_showing && !htrum->old_is_showing)         // to show显示
		{
			__msg("NOT support wake up in intertal cmd 3 ...\n");
			__msg("htrum = %d...\n",htrum);
			_draw_spectrum(htrum);
			//GUI_LyrWinFlushFB( htrum->h_lyr );			//removed  
			_save_locale(htrum);
			htrum->is_internal_cmd = EPDK_FALSE;
			return 0;
		}
		__msg("NOT support wake up in intertal cmd 4 ...\n");
	...
	{
		__msg("NOT support wake up in intertal cmd 5 ...\n");
...
{
	NEW_HTRUM  htrum = (NEW_HTRUM) GUI_CtrlWinGetAddData(hWnd);
	__wrn("\n****_my_GUI_InvalidateRect is start****\n");
	htrum->is_internal_cmd = EPDK_TRUE;
	return 	GUI_InvalidateRect(hWnd, prc, bEraseBkgnd);//gui刷新指定区域
}
...
static __s32  _init_bk(NEW_HTRUM htrum)
{
	...
	htrum->bk_color = 0;
	//	htrum->bk_color = gui_color;			// @131112
	return 0;
}
...
static void   _draw_particle(NEW_HTRUM htrum, __s32 i)
{
	...
	if(length != 0)
	{
		...
		__wrn("dst_pos: x0=%d,y0=%d,x1=%d,y1=%d\n", dst_pos.x0, dst_pos.y0, dst_pos.x1, dst_pos.y1);
...
{
	__s32  i;
	GUI_LyrWinSel(htrum->h_lyr);//选择图层

	for(i = 0; i < htrum->particle_nr; i++)
	{
		__wrn("i = %d\n",i);
		/* draw the buoy */
		_draw_buoy(htrum, i);//绘制频谱移动黄色方块部分
		/* draw particle */
		_draw_particle(htrum, i);//绘制频谱背景图部分
	}
}
