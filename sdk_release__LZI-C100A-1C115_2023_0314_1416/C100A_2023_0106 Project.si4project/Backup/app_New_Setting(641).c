...
/**********************************************************************************
*函数作用：向父窗口APP_NEWSETTING_ID新添加的app应用程序manwin窗口发送按键消息
***********************************************************************************/
...
/**********************************************************************************
*函数作用：新添加app应用manwin窗口的按键处理函数，这个在有framewin窗口是可以不用
***********************************************************************************/
...
static __s32 _app_New_Setting_proc(__gui_msg_t *msg)
{
	...
	switch(msg->id)
	{
		...
		case GUI_MSG_CLOSE:	//关闭窗口，然后返回上一层主窗口界面
		{
			...
			app_new_setting_cmd2parent(msg->h_deswin, SWITCH_TO_OTHER_APP, SETTING_SW_TO_MAIN, 0);//发送到app_root_scene.c 的case APP_NEWSETTING_ID:条件下执行返回到home应用界面
		}
