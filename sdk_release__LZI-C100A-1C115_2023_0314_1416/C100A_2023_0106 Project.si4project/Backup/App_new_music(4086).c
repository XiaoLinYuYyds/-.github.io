...
static __s32 app_new_music_framewin_create(__gui_msg_t *msg)
{
	...
	app_new_music_ctrl_para->new_music_frm_lyr = app_new_music_32bpp_laye_creat(&rect, 1);//创建8bpp图层
	{
...
static __s32 __app_new_music_proc(__gui_msg_t *msg)
{
	...
	switch(msg->id){
		case GUI_MSG_CREATE:{/*0，创建*/
			...
			playlist_framewin_create(msg);
			app_new_music_framewin_create(msg);
			...
			switch(LOWORD(msg->dwAddData1)){
				...
				case NEW_MUSIC_ID:{/*播放器*/
					...
					switch(HIWORD(msg->dwAddData1)){
						case new_music_play_key_exit:{//发送退出命令
							__gui_msg_t mymsg;
							mymsg.id = GUI_MSG_QUIT;
							mymsg.h_deswin = msg->h_deswin;//本窗口的窗口句柄
							GUI_SendMessage(&mymsg);//向本窗口发送信息
						}
						return EPDK_OK;
				...
				case NEW_MUSIC_PLAYLIST_ID:{/*播放列表*/
					switch(HIWORD(msg->dwAddData1)){
						case send_cmd_play_prev_to_PlayDev:{
							...
							{	
								...
								playmsg.dwAddData1 = msg->dwAddData1;//文件索引id
								playmsg.dwAddData2 = msg->dwAddData2;//播放命令
								__wrn("playmsg.dwAddData1 = %d\n",playmsg.dwAddData1);
