...
static	__s32	new_movie_play_file(new_movie_ctrl_t	*new_movie_ctrl, __s32 index, __bool breakplay)
{
	...
	#endif
	#if 1	/*不使用断点，直接播放文件*/
		app_root_play_app_sounds(SHORT_KEY_WAVE_FILE_PLAY);/*app播放声音*/
...
/*视频控制参数初始化*/
static __s32 __new_movie_ctrl_para_init(new_movie_ctrl_t *movie_ctrl)
{
	if(!movie_ctrl)
	{
...
static	__s32	new_movie_ctrl_para_init(new_movie_ctrl_t	*new_movie_ctrl, RECT *vedio_rect)
{
	...
	#if 1	/*获取视频流的文件位置指示器的当前值底部结束*/
	{
		...
		if(0 != ret)
		{
			__wrn("DISP_CMD_LAYER_BOTTOM fail...\n");
		}
		else
		{
			__wrn("DISP_CMD_LAYER_BOTTOM success...\n");
		}
	}
	#endif
...
static	__s32	new_movie_scene_create(__gui_msg_t *msg)
{
	...
	__wrn("new_movie_win->new_movie_win = %x...\n",new_movie_ctrl->new_movie_win);
	
	#if 0	/*图层显示矩形大小1024*600*/
		vedio_rect.x = 0;
		vedio_rect.y = 0;
	...
	#endif
	#if 1	/*视频图层资源申请*/
		__new_movie_ctrl_para_init(new_movie_ctrl);
	...
	#endif
	#if 1	/*播放视频文件*/
		new_movie_start_play(new_movie_ctrl, 0,1);
...
static __s32	_new_movie_manager_win_proc(__gui_msg_t *msg)
{
	...
	__wrn("_new_movie_manager_win_proc: msg->id=%d, msg->h_deswin=%x, msg->dwAddData1=%d, msg->dwAddData2=%d\n"
	...
	case GUI_MSG_CREATE: {/*0,创建*/
		...
		if(enter_key_num == 0){
			...
			msg->h_deswin = NULL;
			__wrn("new_movie_win = %x...\n",new_movie_ctrl->new_movie_win);
			msg->h_deswin = new_movie_ctrl->new_movie_win;*/
		}
		enter_key_num += 1;
		return EPDK_OK;
	}
	case GUI_MSG_DESTROY:{/*1,销毁*/
		...
		#endif
		#if 0	/*删除赋值的manwin窗口结构体new_movie_win数据*/
		...
		#endif
		#if 0	/*清除赋值的manwin窗口结构体数据*/
		if(new_movie_ctrl)/*删除赋值的manwin窗口结构体数据*/
		{
			__wrn("new_movie_ctrl->new_movie_win destory begin is ok...\n");
			new_movie_ctrl->new_movie_win = NULL;
		}
			...
			switch(msg->dwAddData1){
				...
				case GUI_MSG_KEY_ENTER:{/*发送关闭信号*/
					...
					if(enter_key_num == 2){
						...
						sta = robin_get_fsm_status();/*获取状态机目前的播放状态*/

						if(CEDAR_STAT_PLAY == sta)/*如果为播放状态*/
						{
							robin_set_cmd_pause();/*发送暂停命令，暂停播放*/
							__wrn("robin_set_cmd_pause is send pause cmd....\n");
							app_root_play_app_sounds(SHORT_KEY_WAVE_FILE_PAUSE);
						}
						else if(CEDAR_STAT_PAUSE == sta)/*如果为暂停状态*/
						{
							robin_set_cmd_play();/*发送播放命令，开始播放*/
							__wrn("robin_set_cmd_pause is send play cmd....\n");
	...
	case GUI_MSG_TIMER:
	{
				...
				if(new_movie_ctrl)
				{
					...
					if(LOWORD(msg->dwAddData1) == new_movie_ctrl->auto_play_timmer_id)/*0x10*/
					{
						...
						total_time = robin_get_total_time();	/*获取当前播放文件的总时间*/
						__wrn("robin_get_total_time = %d \n",total_time);
						if(cur_time == total_time)//时间相同时暂停播放，并退出
						{	static __s32 time_count=0;
							#if 1/*直接退出*/
							__gui_msg_t	my_msg;
							esKRNL_TimeDly(10);
							__wrn("\n....cur_time == total_time....\n");
							__wrn("send GUI_MSG_QUIT is press....\n");
							my_msg.h_deswin	 =	msg->h_deswin;
							my_msg.h_srcwin	 =	msg->h_srcwin;
							my_msg.id		 =	GUI_MSG_QUIT;/*发送退出信号*/
							my_msg.dwAddData1 = NULL;
							...
							GUI_SendNotifyMessage(&my_msg);
								...
								new_movie_start_play(new_movie_ctrl, 0,time_count);
								__wrn("new_movie_start_play is success.....\n");
							#endif
						}
						/*{
