...
static __s32 new_movie_check_for_play_err(new_movie_ctrl_t *new_movie_ctrl)
{
	...
	if(ret < 0)
	{
		...
		new_movie_exit_movie(new_movie_ctrl, 0);//退出应用
		return EPDK_FAIL;
	}
...
//进度条framewin窗口创建
static __s32 __movie_prog_scene_create(new_movie_ctrl_t *movie_ctrl)
{
	__s32 ret;
...
static __s32 __app_new_movie_proc(__gui_msg_t *msg)
{
	...
	switch(msg->id)
	{
		case GUI_MSG_CREATE://创建窗口
		{
			...
			#endif
				__movie_prog_scene_create(new_movie_ctrl);
			#if 1	/*配置/开启播放程序定时器*/
		...
		case GUI_MSG_CLOSE://关闭，退出窗口
		{
			...
			else//退出到 new explorer
			{
				...
				new_movie_cmd2parent(msg->h_deswin, NEW_SWITCH_TO_OTHER_APP, NEW_MOVIE_SW_TO_NEW_EXPLORER, 0);//退出movie到new_explorer应用程序窗口
			}
		...
		case GUI_MSG_DESTROY://摧毁窗口功能
		{
			...
			if(new_movie_ctrl)
			{
				...
				#if 0	//删除进度条图层
				{
					...
					if(new_movie_ctrl->progressbar_scene)
					{
						if(!scene_para->hparent)
						{
							__err("invalid para..\n");
							return EPDK_FAIL;
						}
	
						GUI_FrmWinDelete(scene_para->hparent);
						...
						eLIBs_memset(scene_para, 0, sizeof(progressbar_scene_t));//防止重复释放
						esMEMS_Bfree(scene_para, sizeof(progressbar_scene_t));
					}
				}
				#endif
