		...
		}
		{
			...
			{//绘制进度条移动
				...
				if(cur_time <= 0 || total_time <= 0){//当前播放时间或总时间小于等于0时
					...
					__here__;
					n = ((new_music_ctrl->new_music_uipara->progressbar[0].width * cur_time)/*进度条背景图片的宽(364)*当前时间*/
...
static __s32  new_music_play_ctrl_key_proc(__gui_msg_t *msg)
{
	...
	/*if(KEY_REPEAT_ACTION == msg->dwAddData2)//
	{
		esKRNL_TimeDly(1000);//延时时间1s
		__wrn("KEY_REPEAT_ACTION\n");
	}*/
