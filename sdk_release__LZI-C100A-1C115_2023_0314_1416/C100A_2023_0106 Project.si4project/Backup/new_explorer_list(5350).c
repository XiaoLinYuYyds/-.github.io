...
static	__s32 New_explorer_ListView_creat(H_WIN list_win)
{
	...
	if(list_para->media_type = RAT_MEDIA_TYPE_AUDIO/*RAT_MEDIA_TYPE_VIDEO*/)		//文件类型为视频文件
	{
...
static __s32 _new_explorer_list_ui_all_paint(__gui_msg_t *msg)
{
	...
	__wrn("list_layer = %d\n",new_explorer_list_ctrl->new_explorer_list_para->list_layer);
		...
		for(list_item_number = 0;list_item_number < 10;list_item_number++)
		{
			...
			{
				list_item_focus_bmp = theme_hdl2buf(new_explorer_list_ctrl->new_explorer_list_item_focus_bmp);//获取选中的背景图片数据
...
*return		：return EPDK_OK;返回一个OK
=============================================================================================*/
static __s32  New_explorer_ListView_Key_Proc(__gui_msg_t *msg,__s32 key_num)
{
	#if 1
	...
	else
	{
		switch(msg->dwAddData1)
		{
			...
			case GUI_MSG_KEY_LONGENTER:{
				app_new_explorer_cmd2parent(msg->h_deswin, ID_OP_NEW_EXPLORER_ENTER, key_num, 0);
			}
			break;
...
static __s32 _new_explorer_list_win_proc(__gui_msg_t *msg)
{
	...
	switch(msg->id)
	{
		...
		{
			__wrn(" GUI_MSG_KEY is press send play file index=%d...\n",key_num);
			New_explorer_ListView_Key_Proc(msg,key_num);//按键处理
			#if 0//直接发送退出信号
				__gui_msg_t mymsg;
				key_num += 1;
				if(key_num  == 3)
