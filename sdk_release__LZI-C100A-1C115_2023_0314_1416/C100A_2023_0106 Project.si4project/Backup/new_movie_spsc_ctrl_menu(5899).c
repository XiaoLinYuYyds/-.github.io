...
static __s32 new_movie_spsc_ctrl_menu_ui_para_init(void)
{
	...
	for(i=0;i<movie_spsc_ctrl_menu_icon_num;i++)
	{
		if(!ui_para->uipara_icon[i].res_theme)//进度条移动图片资源申请
		{
			...
			ui_para->uipara_icon[i].res_theme = dsk_theme_open(ui_para->uipara_icon[i].res_theme);//进度条图
		}
	}
...
void *new_movie_spsc_ctrl_menu_scene_create(new_movie_spsc_ctrl_menu_create_para_t *create_para)
{
	...
	#if 1//控制菜单的图层大小:0，0，364，94
		lyr_rect.x		=	ui_para->uipara_frm.x;
		lyr_rect.y		=	ui_para->uipara_frm.y;
		lyr_rect.width	=	ui_para->uipara_frm.w;
		lyr_rect.height	=	ui_para->uipara_frm.h;
	...
	framewin_para.rect.x		= ui_para->uipara_frm.x;//framewin窗口图层大小和位置: 0，0，364，94
...
{
	new_movie_spsc_ctrl_menu_scene_t *scene_para;
	scene_para = (new_movie_spsc_ctrl_menu_scene_t *)handle;
	__here__;
