...
static void init_headbar_para(__gui_msg_t *p_msg, headbar_para_t *para)
{
	...
	para->bt_rect.x 		= headbar_uipara.bt.x;
	para->bt_rect.y 		= headbar_uipara.bt.y;
	para->bt_rect.width  = headbar_uipara.bt.w;
	para->bt_rect.height = headbar_uipara.bt.h;
	...
	para->bt_htheme		= dsk_theme_open(headbar_uipara.res_id.bt_id);
	...
	para->bt_bmp		= dsk_theme_hdl2buf(para->bt_htheme);
...
static void deinit_headbar_para(headbar_para_t *para)
{
	...
	dsk_theme_close(para->bt_htheme);
}
...
static __s32 headbar_on_paint_argb(__gui_msg_t *msg, headbar_paint_e mode)
{
	...
	if( ((screen_width == 320 )&&( screen_height == 240))||((screen_width == 480 )&&( screen_height == 272))||((screen_width == 800 )&&( screen_height == 480))
	||((screen_width == 1024 )&&( screen_height == 600))||((screen_width == 1024 )&&( screen_height == 768)))
	{
	...
	switch(mode)
	{
		case HB_PAINT_INIT:			/* 全部更新 */
		{
			...
			if(lz_drv_fail_flag)
			{
				GUI_DispStringAt("NG", p_ctrl->vol_rect.x -32, p_ctrl->vol_rect.y-5);
			}
		...
		case HB_PAINT_VOL:			/* 更新电量 */
		{
			...
			/* 画电池图标 */
			{
				...
				if(EPDK_TRUE == p_ctrl->data.charge_sta && EPDK_FALSE == dsk_power_is_full_charge())	//在充电,在电量不满，动态显示电量
				{
				...
				/*
				else if(EPDK_TRUE == p_ctrl->data.charge_sta && EPDK_TRUE == dsk_power_is_full_charge())	//充电，且电量已满
				{
					GUI_BMP_Draw(p_ctrl->vol_bmp[p_ctrl->data.vol_level], p_ctrl->vol_rect.x, p_ctrl->vol_rect.y);
					charge_flag = vol_level;
				}
		...
		case HB_PAINT_BT:
		{
			...
			GUI_ClearRect(
						  p_ctrl->bt_rect.x,
						  p_ctrl->bt_rect.y,
						  p_ctrl->bt_rect.x + p_ctrl->bt_rect.width -1,
						  p_ctrl->bt_rect.y + p_ctrl->bt_rect.height -1
						  );
			if( msg->dwAddData1)
			{
				GUI_BMP_Draw(p_ctrl->bt_bmp, p_ctrl->bt_rect.x, p_ctrl->bt_rect.y);
			}
		}
...
static __s32 cb_headbar_framewin(__gui_msg_t *p_msg)
{
	switch(p_msg->id)
	{
			...
			if(p_msg->dwAddData1 == TBAR_VOL_TIMER_ID)
			{
				...
				if(GUI_LYRWIN_STA_ON == GUI_LyrWinGetSta(lyr))
				{
					...
					if(EPDK_TRUE == p_ctrl->para.data.charge_sta || vol_level != p_ctrl->para.data.vol_level
					    || charge_sta != p_ctrl->para.data.charge_sta)	//加充电判断
					{
						// 更新电池图标
		...
		case HBAR_BT_PAINT:
		{
			headbar_on_paint(p_msg, HB_PAINT_BT);
			return EPDK_OK;
		}
