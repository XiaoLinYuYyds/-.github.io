...
/***********************************************************************************************************
	建立图层
************************************************************************************************************/
H_LYR new_explorer_list_8bpp_layer_create(RECT *rect,__s32 pipe)
{
	H_LYR layer = NULL;
	FB  fb =
	{
	...
	__wrn("New_Explorer_list_8bpp_layer_create CREATE\n");
	return layer;
}

/******************************************************************
*liat列表图层初始化，申请使用图片资源
*******************************************************************/
...
static void New_explorer_list_uninit(new_explorer_list_ctrl_t *list_ctrl)
{
	...
	if(list_ctrl->new_explorer_scroll_bg_bmp)
	{
		dsk_theme_close(list_ctrl->new_explorer_scroll_bg_bmp);//滚动条背景图释放
		list_ctrl->new_explorer_scroll_bg_bmp = NULL;
	}
...
__s32 new_explorer_get_item_text_rect(  RECT *text_rect)
{
	...
	ui_param = new_explorer_get_list_view_ui_param();//
...
static	__s32 New_explorer_FileSearch_module_rat_init(new_explorer_list_para_t	*list_para)
{
	if(list_para->rat.handle == NULL)	//文件搜索句柄为空，进入一次
	{
		...
		{
			__wrn("!!!!!!!!!!!!! open rat File Search failed!!!!!!!!!!!!!!!!!\n");
		}
	}
	else
	{
...
static	__s32 New_explorer_ListView_creat(H_WIN list_win)
{
	...
	list_para->list_font	 = SWFFont;		//列表文本为空
	__wrn("\n~~~~~~~~~~ListView creat is initializing~~~~~~~~~~~\n");

	if(list_para->media_type = RAT_MEDIA_TYPE_VIDEO)		//文件类型为视频文件
	{
		__wrn("\n~~~~~~~~~~file search is initializing~~~~~~~~~~~\n");
		New_explorer_FileSearch_module_rat_init(list_para);	//文件搜索模式,搜索视频文件
		//New_explorer_get_last_para(list_para);
	...
	__wrn("\n~~~~~~~~~~file search is 11111	OK~~~~~~~~~~~\n");
		...
		__wrn("\n~~~~~~~~~~file search is 22222	OK~~~~~~~~~~~\n");
			...
			{
				__wrn("\n~~~~~~~~~~file search is 22222	OK~~~~~~~~~~~\n");
				New_explorer_GetListItemFileName(list_para, list_item_number, FileName);
				...
				__wrn("\n~~~~~~~~~~file search is 22222	OK~~~~~~~~~~~\n");
					...
					GUI_SetDrawMode(GUI_DRAWMODE_NORMAL);
					{
						...
						List_TextRect.y0 = 	list_item_number*35 + 8 + TextRect.y;	//在列表项目条目显示坐标 矩形控件高为35，项目条数量*35，8为列表显示区域坐标y轴起始坐标
						List_TextRect.x1 =  (TextRect.x + TextRect.width);			//文本名显示的矩形宽为固定338
						List_TextRect.y1 =  list_item_number*35 + (TextRect.y + TextRect.height);	//文本名显示的矩形高为固定30，项目条数量*35+30为y1的y轴终止坐标
						__wrn("List_TextRect: x0=%d, y0=%d, x1=%d, y1=%d\n",List_TextRect.x0,List_TextRect.y0,List_TextRect.x1,List_TextRect.y1);

						__wrn("New_explorer_draw_listview_item_text FileName=%s is...!!\n",FileName);
						GUI_DispStringInRect(FileName, &List_TextRect, GUI_TA_VCENTER);//显示文本到矩形
...
static __s32 _new_explorer_list_ui_one_paint(__gui_msg_t *msg)
{
	...
	__s32	list_item_number=0;
	__wrn("_new_explorer_list_ui_one_paint 0 is....\n");
	...
	__wrn("_new_explorer_list_ui_one_paint 1 is....\n");
	...
	__wrn("list_layer = %d\n",new_explorer_list_ctrl->new_explorer_list_para->list_layer);
		...
		#endif
		#if 1	//绘制列表条目
		for(list_item_number = 0;list_item_number < 9;list_item_number++)
		{
			GUI_SetDrawMode(GUI_DRAWMODE_NORMAL);	//绘制模式为正常模式
			...
			New_explorer_list_get_item_number_size(msg,list_item_number);	//项目编号
			{
				list_item_focus_bmp = theme_hdl2buf(new_explorer_list_ctrl->new_explorer_list_item_focus_bmp);//获取滚动条白色图片数据
				if(!list_item_focus_bmp)
				{
					__wrn("list_item_focus_bmp is null \n");
					return EPDK_FAIL ;
				}
				GUI_BMP_Draw(list_item_focus_bmp, gui_list_item_rect.x, gui_list_item_rect.y);
...
static __s32 _new_explorer_list_win_proc(__gui_msg_t *msg)
{
	...
	switch(msg->id)
	{
		case GUI_MSG_CREATE://0，创建功能
		{
			...
			__wrn("_new_explorer_list_win_proc GUI_MSG_CREATE ok\n");
		...
		case GUI_MSG_PAINT:	//2, 绘制功能
			...
			_new_explorer_list_ui_one_paint(msg);
